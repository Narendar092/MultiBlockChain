...
{
    public class JournalTransactions : IProtectedObjects
    {
        static Dictionary<String, List<ProtectedObject>> ModuleProtectedObjects;
        ...
        static JournalTransactions() // Parameterless Constructor Default
        {
            ModuleProtectedObjects = new Dictionary<String, List<ProtectedObject>>();
            ProtectedObject obj1 = new ProtectedObject("ACTransaction_T", new string[] { "JournalTransactions.AddTransaction", "JournalTransactions.PostTransaction" });
            ProtectedObject obj2 = new ProtectedObject("ACJournalEntry_T", new string[] { "JournalTransactions.JournalEntriesProcessing", "JournalTransactions.JournalEntriesProcessingForNonPolicy" });

            ModuleProtectedObjects.Add("ZeeInsurance", new List<ProtectedObject>() { obj1, obj2 });
        }

        public Dictionary<String, List<ProtectedObject>> GetModuleProtectedObjects()
        {
            return ModuleProtectedObjects;
        }
        ...
        public CResult JournalEntriesProcessingForNonPolicy(Dictionary<string, string> oParams)
        {
            ...
            try
            {
                ...
                var JournalTemplateList = oData.oSubStructureI("Journal Template").oBOIList;
                ...
                if (JournalTemplateList != null)
                {
                    ...
                    foreach (var oJournalTemplate in JournalTemplateList)
                    {
                        ...
                        XIIBO oboi = new XIIBO(TXInitiation);
                        ...
                        if (iJEType == 10 && sTransactionCode.ToLower() == "crecpart")
                            oboi.SetAttribute("FKiACPolicyID", "0");
                        else
                        ...
                        if (!string.IsNullOrEmpty(oParams["rUnReconciled"]))
                            oboi.SetAttribute("rUnReconciled", oParams["rUnReconciled"]);
                        else
                            oboi.SetAttribute("rUnReconciled", rJEAmount.ToString());
                        ...
                        if (oboi.AttributeI("rUnReconciled").doValue == 0)
                            oboi.SetAttribute("iReconciled", 30.ToString());
                        else
                        ...
                        oboi.iUserID = Convert.ToInt32(FKiUserID);
                ...
                {
                    oCResult.sMessage = "ERROR: [CRITICAL ALERT  There is no Journal templates for the transactiontype " + oParams["sTransCode"] + "] \r\n PolicyID:" + oParams["FKiACPolicyID"] + ", sAnalysis: " + sAnalysis;
                    oCResult.xiStatus = xiEnumSystem.xiFuncResult.xiError;
                    oCResult.LogToFile();
                    oCResult.sMessage = "ERROR: [CRITICAL ALERT  There is no Journal templates for the transactiontype " + oParams["sTransCode"] + "] \r\n PolicyID:" + oParams["FKiACPolicyID"] + ", sAnalysis: " + sAnalysis;
                    oIB.SaveErrortoDB(oCResult);
        ...
        public decimal RealRound(decimal dValue)
        {
            ...
            int DotIndex = sAmount.IndexOf(".");
            if (sAmount.Contains(".") && (sAmount.Length - (DotIndex + 1)) > 2 && double.Parse(sAmount.Substring(sAmount.Length - 1)) > 4)
            {
                bRoundUp = true;
        ...
        public void Calculation(string sTransCode)
        {
            ...
            if (bIPT)
            {
                ...
                zTaxPC = (zTaxRate != null && zTaxRate != 0) ? Convert.ToDecimal(zTaxRate) : zTaxPC;
                if (iTaxType == 10)
                {
            ...
            {
                if (sTransCode == "DNEWP" || sTransCode == "DADDP" || sTransCode == "DRNWP" || sTransCode == "DRETP" || sTransCode == "DRBRP" || sTransCode == "CNODNEWP" || sTransCode == "CNODADDP" || sTransCode == "CNODRNWP" || sTransCode == "CNODRETP" || sTransCode == "CNODRBRP")
                {
                    rCommission = rPremiumT;
                }
            }
        ...
        public CResult AddTransaction(int iNewPolStatus, int iTransTypeCode, string sTransactionCode, int iRefTypeID, string sSessionID, string sCurrentGUID, double? rUReconciled = null)
        {
            CResult oCResult = new CResult();
            CResult oCR = new CResult();
            XIInstanceBase oIB = new XIInstanceBase();
            ...
            try
            {
                sAnalysis = new StringBuilder();
                oCResult.sClassName = oCResult.Get_Class();
                oCResult.sFunctionName = System.Reflection.MethodBase.GetCurrentMethod().Name;
                oCResult.xiStatus = xiEnumSystem.xiFuncResult.xiInProcess;
                Calculation(sTransCode);
                ...
                XIIBO oboiACT = new XIIBO(TXInitiation);
                ...
                oboiACT.SetAttribute("zTaxRate", zTaxRate.ToString());
                //post the transactions for a new policy
                oboiACT.iUserID = Convert.ToInt32(FKiUserID);
                ...
                if (rPremium != 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            oboiACT = new XIIBO(TXInitiation);
                            oxidACT = new XIDXI();
                            ...
                            else
                            {
                                ...
                                oBOIPolicyVersion.iUserID = Convert.ToInt32(FKiUserID);
                        ...
                        dictnparams.Add("rUnReconciled", rUReconciled.ToString());
                        ...
                        oboiACT.SetAttribute("sAnalysis", sAnalysis.ToString());
                        var sUpdateAnalysis = oboiACT.Save(oboiACT);
        ...
        public CResult Get_ProductVersion(DateTime dEffectiveFrom)
        {
            ...
            try
            {
                ...
                nParams.AddRange(nParams.Where(m => m.sType == "Structure").ToList());
                var sDataSource = oDXI.GetBODataSource(oBOD.iDataSourceXIGUID.ToString(), oBOD.FKiApplicationID);
        ...
        public CResult PostTransaction(XIIBO oTransaction, string sDataBase, string sSessionID = "", string sCurrentGUID = "", bool bLocalnoEDI = false, string sUserID = "", double? rUReconciled = null)
        {
            CResult oCResult = new CResult();
            try
            {
                ...
                if (oTransaction.AttributeI("zBaseValue").doValue != 0)
                {
                    ...
                    dCoverStart = oTransaction.AttributeI("dCoverStart").sValue == null ? DateTime.Now.ToString(XIConstant.Date_Format) + " " + DateTime.Now.ToString(XIConstant.Time_Format) : oTransaction.AttributeI("dCoverStart").sValue;
                    ...
                    zTaxRate = !string.IsNullOrEmpty(oTransaction.AttributeI("zTaxRate").sValue) ? (decimal?)oTransaction.AttributeI("zTaxRate").doValue : null;
                    //get transation type info here
                    List<CNV> oWhere = new List<CNV>();
                    oWhere.Add(new CNV { sName = "sCode", sValue = oTransaction.AttributeI("sTransCode").sValue });
                    oData = oIXI.BOI("ACTransType_T", null, null, oWhere).Structure("TRANTYPE", FKiUserID).XILoad("", false, FKiUserID);
                    ...
                    else
                    {
                        ...
                        sNameACTT = string.IsNullOrEmpty(oTransaction.AttributeI("sName").sValue) ? OTransType.AttributeI("sName").sValue : OTransType.AttributeI("sName").sValue.ToLower() == oTransaction.AttributeI("sName").sValue.ToLower() ? oTransaction.AttributeI("sName").sValue : OTransType.AttributeI("sName").sValue;
                    ...
                    if (bLocalnoEDI == false)
                    {
                        ...
                        if (sTransactionCode == "ADDON")
                        {
                            ...
                            if (oTransaction.AttributeI("FKiProductAddOnID").iValue != 0)
                            {
                                ...
                                oCResult = AddTransaction(iNewPolStatus, iTransTypeCode, sTransactionCode, refTypeID, sSessionID, sCurrentGUID, rUReconciled);
                                if (!oCResult.bOK && oCResult.oResult == null)
                                {
                    ...
                    else
                    {
                        ...
                        oboiAACT.iUserID = Convert.ToInt32(FKiUserID);
                        ...
                        else
                        {
                            ...
                            //Admin Transaction
                            oboiAACT = new XIIBO(TXInitiation);
                            oxidAACT = new XIDXI();
        ...
        public CResult TransactionPopup(List<CNV> oParams)
        {
            ...
            try
            {
                ...
                var iQSIID = oParams.Where(t => t.sName == "iQSIID").Select(f => f.sValue).FirstOrDefault();
        ...
        public CResult NonPolicyTransactions(List<CNV> oParams)
        {
            ...
            try
            {
                ...
                var oQSInstance = oXI.GetQuestionSetInstanceByID("0", iQSIID.ToString(), null, 0, 0, "");
                //FKsUserID = GetUser(oParams);
                // Perform Trasactions here 
                XIIBO oTransaction = new XIIBO();
                XIIXI oIXI = new XIIXI();
                ...
                {
                    oQSInstance = oCache.Get_QuestionSetCache("QuestionSetCache", sCurrentGUID, iQSIID.ToString());
                    foreach (var item in oQSInstance.Steps.Last().Value.Sections.FirstOrDefault().Value.FormValues)
                    {
                        oTransaction.SetAttribute(item.sName, item.sValue);
                    }
                    string ClientID = oCache.Get_ParamVal(sSessionID, sCurrentGUID, null, "{XIP|Customer_T.id}");
                    oTransaction.SetAttribute("FKiClientID", ClientID);
                    oTransaction.SetAttribute("FKsWhoID", sUserID);
                // TRANSACTION SCOPE HERE
                oCResult = PostTransaction(oTransaction, oParams.Where(t => t.sName == "sDataBase").Select(f => f.sValue).FirstOrDefault(), sSessionID, sCurrentGUID, false);
                    if (oQSInstance.Steps.Last().Value.Sections.FirstOrDefault().Value.FormValues.Any(X => X.sValue == "CREC" || X.sValue == "CREC2"))
                    {
                        XIIBO oBOI = new XIIBO();
                        XIDBO oBOD = new XIDBO();
                        oBOD = (XIDBO)oCache.GetObjectFromCache(XIConstant.CacheBO, "ACReconcilliation_T");
                        oBOI.BOD = oBOD;
                        oBOI.Attributes["sName".ToLower()] = new XIIAttribute { sName = "sName".ToLower(), sValue = "Client Reconciliation", bDirty = true };
                        oBOI.Attributes["refAccountCategory".ToLower()] = new XIIAttribute { sName = "refAccountCategory".ToLower(), sValue = oQSInstance.Steps.Last().Value.Sections.FirstOrDefault().Value.FormValues.Where(X => X.sName == "refAccountCategory").Select(t => t.sValue).FirstOrDefault(), bDirty = true };
                        oBOI.Attributes["iType".ToLower()] = new XIIAttribute { sName = "iType".ToLower(), sValue = "0", bDirty = true };
                        ...
                        oBOI.Attributes["rTotalPaid".ToLower()] = new XIIAttribute { sName = "rTotalPaid".ToLower(), sValue = oQSInstance.Steps.Last().Value.Sections.FirstOrDefault().Value.FormValues.Where(X => X.sName == "zBaseValue").Select(t => t.sValue).FirstOrDefault(), bDirty = true };
                        oBOI.Attributes["dtPosted".ToLower()] = new XIIAttribute { sName = "dtPosted".ToLower(), sValue = DateTime.Now.ToString(XIConstant.Date_Format) + " " + DateTime.Now.ToString(XIConstant.Time_Format), bDirty = true };
                        oBOI.Attributes["rClosingBalance".ToLower()] = new XIIAttribute { sName = "rClosingBalance".ToLower(), sValue = oQSInstance.Steps.Last().Value.Sections.FirstOrDefault().Value.FormValues.Where(X => X.sName == "zBaseValue").Select(t => t.sValue).FirstOrDefault(), bDirty = true };
                        //FKiACTransactionID 
                        oBOI.Attributes["FKiACTransactionID".ToLower()] = new XIIAttribute { sName = "FKiACTransactionID".ToLower(), sValue = ((XIIBO)oCResult.oResult).Attributes["id"].sValue, bDirty = true };
                        ...
                        oCache.Set_ParamVal(sSessionID, sCurrentGUID, null, "{XIP|ACTransaction_T.id}", ((XIIBO)oCResult.oResult).Attributes["id"].sValue, null, new List<CNV>());
                    }
                    oCache.Set_ParamVal(sSessionID, sCurrentGUID, null, "{XIP|refAccountCategory}", oQSInstance.Steps.Last().Value.Sections.FirstOrDefault().Value.FormValues.Where(X => X.sName == "refAccountCategory").Select(t => t.sValue).FirstOrDefault(), null, new List<CNV>());
                }
                oCResult.xiStatus = xiEnumSystem.xiFuncResult.xiSuccess;
            }
            catch (Exception ex)
        ...
        public CResult ReverseTransaction(List<CNV> oParams)//int iReversalType, string TransType = "REVB", int PolicyID = 100)
        {
            ...
            //FKsUserID = GetUser(oParams);
            var oQSInstance = oXI.GetQSXIValuesByQSIID(iQSIID.ToString());
            try
            {
                ...
                CResult oCresult = new CResult();
                oboi = BOI.GetCopy();
                oboi.Exclude("id,dWhen,rBalance,rAllocatedAgainst,rAllocatedToByOthers,XIGUID");
                ...
                oSQLParams.Add(new SqlParameter { ParameterName = "@FKiTransactionID", Value = oParams.Where(t => t.sName == "{XIP|ACTransaction_T.id}").Select(f => f.sValue).FirstOrDefault().ToString() });
                ...
                foreach (var item in oJEBOI)
                {
                    ...
                    item.iUserID = Convert.ToInt32(FKiUserID);
                    res = item.Save(item);
                    if (!res.bOK && res.oResult == null)
                    {
        ...
        public CResult Update_PolicyBalance(int PolicyID, int iUpdateMe, int iType = 0)
        {
            ...
            for (j = 1; j <= 2; j++)
            {
                ...
                sWhereClause.Append(" AND " + Journal + ".[FKiACPolicyID] =" + PolicyID.ToString());
                if (iType == 10)
                {
            ...
            if (iType == 10)
            {
                ...
                if (iUpdateMe == 10)
                {
                    oBOIPolicy.iUserID = Convert.ToInt32(FKiUserID);
        ...
        public CResult CopyTransaction(List<CNV> oParams)
        {
            ...
            try
            {
                ...
                XIIBO oboi = new XIIBO();
                oboi = BOI.GetCopy();
                oboi.Exclude("id,iStatus,XIGUID");
                ...
                else
                {
                    ...
                    if (oBOIJournalsList.Count() > 0)
                    {
                        ...
                        {
                            item.Exclude("id,iStatus,FKiTransactionID,XIGUID");
        ...
        public Dictionary<string, double> Update_rBalance(int iAccountID)//, string pstrAttr = null)
        {
            ...
            string Select = "select sum(rAmount) from ACJournalEntry_T with(nolock) where ";
                sADWhere = "FKiAccountID=" + iAccountID + " and (iStatus=0 or iStatus=30)  and iReconciled<30"; //'&FKiEnterpriseID=" & iPKEnt
                if (oACCAccount.AttributeI("iOverrideType").sValue == "10")
                    sADWhere += "and iReconcilliationType = 0";
                else
                    sADWhere += "and iReconcilliationType = 10";
            ...
            // Update client balance where applicable
            XIIXI oIXICustomer = new XIIXI();
            if (oACCAccount.Attributes["FkiClientID"].iValue.ToString() != null && oACCAccount.Attributes["FkiClientID"].iValue.ToString() != "0")
            {
                var oCustomer = oIXI.BOI("Customer_T", oACCAccount.Attributes["FkiClientID"].iValue.ToString());
                oCustomer.SetAttribute("rAccountBalance", rBalance.ToString());
                oCResult = oCustomer.Save(oCustomer);
            }
            }
        ...
        public CResult TransactionCorrection(List<CNV> oParams)
        {
            CResult oCResult = new CResult();
            try
            {
                ...
                string sCurrentGUID = oParams.Where(t => t.sName == "sGUID").Select(f => f.sValue).FirstOrDefault();
                string sUserID = oParams.Where(m => m.sName.ToLower() == "iUserID".ToLower()).Select(m => m.sValue).FirstOrDefault();
                string iACPolicyID = oParams.Where(m => m.sName.ToLower() == "{XIP|iACPolicyID}".ToLower()).Select(m => m.sValue).FirstOrDefault();
                string iQuoteID = oParams.Where(t => t.sName == "{XIP|iQuoteID}").Select(f => f.sValue).FirstOrDefault();
                string sDataBase = oParams.Where(t => t.sName == "sDataBase").Select(f => f.sValue).FirstOrDefault();
                ...
                Guid.TryParse(sQSInstanceID, out QSIXIGUID);
                XIIXI oIXI = new XIIXI();
                var oBOI = oIXI.BOI("ACPolicy_T", iACPolicyID);
                var DefaultAdmin = oIXI.BOI("Aggregations", iQuoteID, "FKiQSInstanceIDXIGUID,zDefaultAdmin,rPaymentCharge,rInsurerCharge,rPrice,rTotal,rSubTotal,zCommissionOverride,iCommissionType,rPayableAmount,bISPaymentToInsurer,rFinalCommission,rOverrideAdmin,rAddonPrice,sCode,iPaymentMethodType,iTaxType,zTaxRate");
                XIIBO oTransaction = new XIIBO();
                ...
                oTransaction.SetAttribute("FKiUserID", oBOI.AttributeI("FKiUserID").sValue);
                oTransaction.SetAttribute("FKsWhoID", sUserID);
                ...
                oTransaction.SetAttribute("zTaxRate", DefaultAdmin.AttributeI("zTaxRate").doValue.ToString());
                oTransaction.SetAttribute("iTaxType", DefaultAdmin.AttributeI("iTaxType").doValue.ToString());
                string sTranstype = oCache.Get_ParamVal(sSessionID, sCurrentGUID, null, "{XIP|TransactionType}");
                ...
                oWhrParams.Add(new CNV { sName = "FKiQuoteID", sValue = iQuoteID });
                Dictionary<string, XIIBO> oXIBOI = new Dictionary<string, XIIBO>();
                QueryEngine oQE = new QueryEngine();
                string sWhereCondition = "FKiQuoteID =" + iQuoteID + ",XIDeleted = 0";
                var oQResult = oQE.Execute_QueryEngine("ACTransaction_T", "*", sWhereCondition);
                if (oQResult.bOK && oQResult.oResult != null)
                {
                    oXIBOI = (Dictionary<string, XIIBO>)oQResult.oResult;
                }
                XIIBO CNote = new XIIBO();
                double rUnReconciled = 0;
                foreach (var item in oXIBOI.Values.ToList())
                {
                    CNote = new XIIBO();
                    CNote = item;
                    CNote.SetAttribute("FKiUserID", oBOI.AttributeI("FKiUserID").sValue);
                    CNote.SetAttribute("FKsWhoID", sUserID);
                    if (item.AttributeI("sTransCode").sValue.ToLower() == "newp")
                    {
                        CNote.SetAttribute("sTransCode", "CNONEWP");
                        rUnReconciled = item.AttributeI("zBaseValue").doValue;
                        oCResult = PostTransaction(CNote, sDataBase, sSessionID, sCurrentGUID, false, sUserID, rUnReconciled);
                        AutoReconciliation(CNote, item.AttributeI("id").sValue, ((XIIBO)oCResult.oResult).AttributeI("id").sValue);
                        oTransaction.SetAttribute("sTransCode", "NEWP");
                        ...
                        rUnReconciled = DefaultAdmin.AttributeI("rPrice").doValue;
                        oCResult = PostTransaction(oTransaction, sDataBase, sSessionID, sCurrentGUID, false, sUserID, rUnReconciled);
                    }
                    if (item.AttributeI("sTransCode").sValue.ToLower() == "dnewp")
                    {
                        CNote.SetAttribute("sTransCode", "CNODNEWP");
                        rUnReconciled = item.AttributeI("zBaseValue").doValue;
                        oCResult = PostTransaction(CNote, sDataBase, sSessionID, sCurrentGUID, false, sUserID, rUnReconciled);
                        AutoReconciliation(CNote, item.AttributeI("id").sValue, ((XIIBO)oCResult.oResult).AttributeI("id").sValue);
                        oTransaction.SetAttribute("sTransCode", "DNEWP");
                        oTransaction.SetAttribute("zBaseValue", (DefaultAdmin.AttributeI("rFinalCommission").doValue).ToString());
                        oTransaction.SetAttribute("iSystemType", "100");
                        rUnReconciled = DefaultAdmin.AttributeI("rFinalCommission").doValue;
                        oCResult = PostTransaction(oTransaction, sDataBase, sSessionID, sCurrentGUID, false, sUserID, rUnReconciled);
                    }
                    if (item.AttributeI("sTransCode").sValue.ToLower() == "addp")
                    {
                        CNote.SetAttribute("sTransCode", "CNOADDP");
                        rUnReconciled = item.AttributeI("zBaseValue").doValue;
                        oCResult = PostTransaction(CNote, sDataBase, sSessionID, sCurrentGUID, false, sUserID, rUnReconciled);
                        AutoReconciliation(CNote, item.AttributeI("id").sValue, ((XIIBO)oCResult.oResult).AttributeI("id").sValue);
                        oTransaction.SetAttribute("sTransCode", "ADDP");
                        oTransaction.SetAttribute("zBaseValue", (DefaultAdmin.AttributeI("rPrice").doValue).ToString());
                        oTransaction.SetAttribute("iSystemType", "100");
                        rUnReconciled = DefaultAdmin.AttributeI("rPrice").doValue;
                        oCResult = PostTransaction(oTransaction, sDataBase, sSessionID, sCurrentGUID, false, sUserID, rUnReconciled);
                    }
                    if (item.AttributeI("sTransCode").sValue.ToLower() == "daddp")
                    {
                        CNote.SetAttribute("sTransCode", "CNODADDP");
                        rUnReconciled = item.AttributeI("zBaseValue").doValue;
                        oCResult = PostTransaction(CNote, sDataBase, sSessionID, sCurrentGUID, false, sUserID, rUnReconciled);
                        AutoReconciliation(CNote, item.AttributeI("id").sValue, ((XIIBO)oCResult.oResult).AttributeI("id").sValue);
                        oTransaction.SetAttribute("sTransCode", "DADDP");
                        oTransaction.SetAttribute("zBaseValue", (DefaultAdmin.AttributeI("rFinalCommission").doValue).ToString());
                        oTransaction.SetAttribute("iSystemType", "100");
                        rUnReconciled = DefaultAdmin.AttributeI("rFinalCommission").doValue;
                        oCResult = PostTransaction(oTransaction, sDataBase, sSessionID, sCurrentGUID, false, sUserID, rUnReconciled);
                    }
                    if (item.AttributeI("sTransCode").sValue.ToLower() == "rnwp")
                    {
                        CNote.SetAttribute("sTransCode", "CNORNWP");
                        rUnReconciled = item.AttributeI("zBaseValue").doValue;
                        oCResult = PostTransaction(CNote, sDataBase, sSessionID, sCurrentGUID, false, sUserID, rUnReconciled);
                        AutoReconciliation(CNote, item.AttributeI("id").sValue, ((XIIBO)oCResult.oResult).AttributeI("id").sValue);
                        oTransaction.SetAttribute("sTransCode", "RNWP");
                        oTransaction.SetAttribute("zBaseValue", (DefaultAdmin.AttributeI("rPrice").doValue).ToString());
                        oTransaction.SetAttribute("iSystemType", "100");
                        rUnReconciled = DefaultAdmin.AttributeI("rPrice").doValue;
                        oCResult = PostTransaction(oTransaction, sDataBase, sSessionID, sCurrentGUID, false, sUserID, rUnReconciled);
                    }
                    if (item.AttributeI("sTransCode").sValue.ToLower() == "drnwp")
                    {
                        CNote.SetAttribute("sTransCode", "CNODRNWP");
                        rUnReconciled = item.AttributeI("zBaseValue").doValue;
                        oCResult = PostTransaction(CNote, sDataBase, sSessionID, sCurrentGUID, false, sUserID, rUnReconciled);
                        AutoReconciliation(CNote, item.AttributeI("id").sValue, ((XIIBO)oCResult.oResult).AttributeI("id").sValue);
                        oTransaction.SetAttribute("sTransCode", "DRNWP");
                        oTransaction.SetAttribute("zBaseValue", (DefaultAdmin.AttributeI("rFinalCommission").doValue).ToString());
                        oTransaction.SetAttribute("iSystemType", "100");
                        rUnReconciled = DefaultAdmin.AttributeI("rFinalCommission").doValue;
                        oCResult = PostTransaction(oTransaction, sDataBase, sSessionID, sCurrentGUID, false, sUserID, rUnReconciled);
                    }
                    if (item.AttributeI("sTransCode").sValue.ToLower() == "rbrp")
                    {
                        CNote.SetAttribute("sTransCode", "CNORBRP");
                        rUnReconciled = item.AttributeI("zBaseValue").doValue;
                        oCResult = PostTransaction(CNote, sDataBase, sSessionID, sCurrentGUID, false, sUserID, rUnReconciled);
                        AutoReconciliation(CNote, item.AttributeI("id").sValue, ((XIIBO)oCResult.oResult).AttributeI("id").sValue);
                        oTransaction.SetAttribute("sTransCode", "RBRP");
                        oTransaction.SetAttribute("zBaseValue", (DefaultAdmin.AttributeI("rPrice").doValue).ToString());
                        oTransaction.SetAttribute("iSystemType", "100");
                        rUnReconciled = DefaultAdmin.AttributeI("rPrice").doValue;
                        oCResult = PostTransaction(oTransaction, sDataBase, sSessionID, sCurrentGUID, false, sUserID, rUnReconciled);
                    }
                    if (item.AttributeI("sTransCode").sValue.ToLower() == "drbrp")
                    {
                        CNote.SetAttribute("sTransCode", "CNODRBRP");
                        rUnReconciled = item.AttributeI("zBaseValue").doValue;
                        oCResult = PostTransaction(CNote, sDataBase, sSessionID, sCurrentGUID, false, sUserID, rUnReconciled);
                        AutoReconciliation(CNote, item.AttributeI("id").sValue, ((XIIBO)oCResult.oResult).AttributeI("id").sValue);
                        oTransaction.SetAttribute("sTransCode", "DRBRP");
                        oTransaction.SetAttribute("zBaseValue", (DefaultAdmin.AttributeI("rFinalCommission").doValue).ToString());
                        oTransaction.SetAttribute("iSystemType", "100");
                        rUnReconciled = DefaultAdmin.AttributeI("rFinalCommission").doValue;
                        oCResult = PostTransaction(oTransaction, sDataBase, sSessionID, sCurrentGUID, false, sUserID, rUnReconciled);
                    }
                    if (item.AttributeI("sTransCode").sValue.ToLower() == "admin")
                    {
                        CNote.SetAttribute("sTransCode", "CNOADMIN");
                        oCResult = PostTransaction(CNote, sDataBase, sSessionID, sCurrentGUID, false);
                        AutoReconciliation(CNote, item.AttributeI("id").sValue, ((XIIBO)oCResult.oResult).AttributeI("id").sValue);
                        ...
                        oCResult = PostTransaction(oTransaction, sDataBase, sSessionID, sCurrentGUID, false);
                    }
                }
                oCResult.xiStatus = xiEnumSystem.xiFuncResult.xiSuccess;
            }
        ...
        #endregion
        public CResult AutoReconciliation(XIIBO oTransactionI = null, string sTransactionID = "", string sNewTransactionID = "")
        {
            CResult oCResult = new CResult();
            try
            {
                ...
                oBOD = (XIDBO)oCache.GetObjectFromCache(XIConstant.CacheBO, "ACReconcilliation_T");
                oBOI.BOD = oBOD;
                oBOI.Attributes["sName".ToLower()] = new XIIAttribute { sName = "sName".ToLower(), sValue = "Auto Reconciliation", bDirty = true };
                oBOI.Attributes["refAccountCategory".ToLower()] = new XIIAttribute { sName = "refAccountCategory".ToLower(), sValue = oTransactionI.AttributeI("refAccountCategory").sValue, bDirty = true };
                oBOI.Attributes["iType".ToLower()] = new XIIAttribute { sName = "iType".ToLower(), sValue = "0", bDirty = true };
                oBOI.Attributes["FKsUserID".ToLower()] = new XIIAttribute { sName = "FKsUserID".ToLower(), sValue = oTransactionI.AttributeI("FKsWhoID").sValue, bDirty = true };
                oBOI.Attributes["iStatus".ToLower()] = new XIIAttribute { sName = "iStatus".ToLower(), sValue = "20", bDirty = true };
                oBOI.Attributes["rTotalPaid".ToLower()] = new XIIAttribute { sName = "rTotalPaid".ToLower(), sValue = oTransactionI.AttributeI("zBaseValue").sValue, bDirty = true };
                oBOI.Attributes["dtPosted".ToLower()] = new XIIAttribute { sName = "dtPosted".ToLower(), sValue = DateTime.Now.ToString(XIConstant.Date_Format) + " " + DateTime.Now.ToString(XIConstant.Time_Format), bDirty = true };
                oBOI.Attributes["rClosingBalance".ToLower()] = new XIIAttribute { sName = "rClosingBalance".ToLower(), sValue = oTransactionI.AttributeI("zBaseValue").sValue, bDirty = true };
                //FKiACTransactionID 
                oBOI.Attributes["FKiACTransactionID".ToLower()] = new XIIAttribute { sName = "FKiACTransactionID".ToLower(), sValue = sNewTransactionID, bDirty = true };
                var oTX = oBOI.Save(oBOI);
                string iPKID = oBOI.Attributes.Values.Where(m => m.sName.ToLower() == oBOI.BOD.sPrimaryKey.ToLower()).Select(m => m.sValue).FirstOrDefault();

                XID1Click o1Click = (XID1Click)oCache.GetObjectFromCache(XIConstant.Cache1Click, null, "C90EA621-5C7A-4903-98A4-64608CDF02A4");
                XID1Click o1Click2 = (XID1Click)o1Click.Clone(o1Click);
                oBOD = (XIDBO)oCache.GetObjectFromCache(XIConstant.CacheBO, null, o1Click.BOID.ToString());
                XIDXI oXID = new XIDXI();
                var DataSource = oXID.GetBODataSource(oBOD.iDataSourceXIGUID.ToString(), oBOD.FKiApplicationID);
                List<CNV> nParms = new List<CNV>();
                CNV oNVParams = new CNV();
                oNVParams.sName = "{XIP|FKiACTransactionID}";
                oNVParams.sValue = sTransactionID.ToString();
                nParms.Add(oNVParams);
                var o1ClickResult = new Dictionary<string, XIIBO>();
                o1Click2.ReplaceFKExpressions(nParms);
                o1Click2.sConnectionString = DataSource;
                o1ClickResult = o1Click2.OneClick_Execute();
                XIIBO oACXRecon = new XIIBO();
                if (o1ClickResult.Count > 0)
                {
                    XIDXI oxid = new XIDXI();
                    XIDBO obod = (XIDBO)oxid.Get_BODefinition("ACXReconcilliation_T").oResult;
                    XIIXI oIXI = new XIIXI();
                    XIDBO oJournalBod = (XIDBO)oxid.Get_BODefinition("ACJournalEntry_T").oResult;
                    foreach (var item in o1ClickResult.Values.ToList())
                    {
                        oACXRecon = new XIIBO();
                        oACXRecon.BOD = obod;
                        oACXRecon.SetAttribute("FKiACJEID", item.AttributeI("JournalID").sValue);
                        oACXRecon.SetAttribute("FKiACTransactionID", item.AttributeI("id").sValue);
                        oACXRecon.SetAttribute("rDR", item.AttributeI("DR").sValue);
                        oACXRecon.SetAttribute("rCR", item.AttributeI("CR").sValue);
                        oACXRecon.SetAttribute("FKiOrgID", item.AttributeI("FKiOrgID").sValue);
                        oACXRecon.SetAttribute("FKiEnterpriseID", item.AttributeI("FKiEnterpriseID").sValue);
                        oACXRecon.SetAttribute("FKiSupplierID", item.AttributeI("FKiSupplierID").sValue);
                        oACXRecon.SetAttribute("refAccountCategory", item.AttributeI("refAccountCategory").sValue);
                        oACXRecon.SetAttribute("iType", "0");
                        oACXRecon.SetAttribute("FKiACReconcilliationID", iPKID.ToString());
                        oACXRecon.SetAttribute("iDRCRType", item.AttributeI("iType").sValue);
                        oACXRecon.SetAttribute("FKiOrgID", item.AttributeI("FKiOrgID").sValue);
                        oACXRecon.SetAttribute("rAmountReconciled", (item.AttributeI("DR").sValue != "0" && item.AttributeI("DR").sValue != null) ? item.AttributeI("DR").sValue : item.AttributeI("CR").sValue);
                        var res = oACXRecon.Save(oACXRecon);

                        XIIBO AcJEID = oIXI.BOI("ACJournalEntry_T", item.AttributeI("JournalID").sValue);
                        AcJEID.BOD = oJournalBod;
                        AcJEID.SetAttribute("iReconciled", "30");
                        AcJEID.SetAttribute("rUnReconciled", "0");
                        var resJournal = AcJEID.Save(AcJEID);
                    }
                }
                oCResult.xiStatus = xiEnumSystem.xiFuncResult.xiSuccess;
            }
        ...
        //{
        //    var LeadBOD = (XIDBO)oCache.GetObjectFromCache(XIConstant.CacheBO, "ACPolicy_T", null);
        //    XIIBO oBOI = new XIIBO();
        //    oBOI.BOD = LeadBOD;
        //    oBOI.Save(oBOI);
        //    return "Success";
        //}
        //public string Check_objProtection()
        //{
        //    var LeadBOD = (XIDBO)oCache.GetObjectFromCache(XIConstant.CacheBO, "ACPolicy_T", null);
        //    XIIBO oBOI = new XIIBO();
        //    oBOI.BOD = LeadBOD;
