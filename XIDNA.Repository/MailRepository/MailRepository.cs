...
namespace XIDNA.Repository
{
    public class MailRepository : IMailRepository
    {
        ...
        public List<List<string>> ExtractEmailData(int InboundID, int OrgID, string sDatabase, int iUserID, string sOrgName)
        {
            ...
            return SleadDetails;
            //string iResult = EmailExtractedIntoInstance(NewValues, InboundID, FKiOrgID, database, SubscriptionID);
            //if (iResult == "Success")
            //{
            //    var SleadDetails = EmailExtractedIntoLead(NewValues, InboundID, FKiOrgID, database, SubscriptionID, Status);
            //    var History = dbcontext.ImportHistories.Find(FileID);
                    ...
                    if (reader1.HasRows)
                    {
                        while (reader1.Read())
                        {
                            ...
                            for (int i = 0; i < count; i++)
                            {
                                ...
                                //    Com.SendUserRegisterMail(EmailID, "Register", database, FKiOrgID);
                                //}
                            }
        ...
        private void AssignUserID(long LeadID, int OrgID, string sDatabase)
        {
            ...
            try
            {
                ...
                var AllUsers = dbCore.XIAppUsers.Where(m => m.FKiOrgID == OrgID).ToList();
        ...
        public List<List<string>> InsertAPIDataToInbound(Leads Model, int iUserID, string sOrgName, string sDatabase)
        {
            ...
            if (NewSUb != null)
            {
                var User = dbCore.XIAppUsers.Where(m => m.FKiOrgID == NewSUb.OrganizationID).FirstOrDefault();
        ...
        public DTResponse AppNotificationsGrid(jQueryDataTableParamModel param, int OrgID, string database)
        {
            ...
            AllReports = (from r in Spdb.AppNotifications.Where(m => m.OrganizationID == OrgID)
                          select new VMAppNotifications
                          {
                              ...
                              //RoleName = dbcontext.AspNetGroups.Where(m => m.OrganizationID == FKiOrgID && m.Id == r.RoleID).Select(m => m.RoleName).FirstOrDefault(),
                              //RoleName = GetRoleName(r.RoleID),
        ...
        public List<VMDropDown> GetOrgRoles(int OrgID, string sDatabase)
        {
            ...
            AllRoles = (from c in dbCore.XIAppRoles.Where(m => m.FKiOrgID == OrgID).ToList()
        ...
        public List<VMDropDown> GetUsers(int OrgID, string sDatabase)
        {
            ...
            AllUsers = (from c in dbCore.XIAppUsers.ToList().Where(m => m.FKiOrgID == OrgID)
