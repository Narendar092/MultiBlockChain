...
namespace XIDNA.Repository
{
    public class OrganizationRepository : IOrganizationRepository
    {
        ...
        {
            ModelDbContext dbCore = new ModelDbContext(database);
            var sOrgDB = dbCore.XIAppUsers.Where(m => m.FKiOrgID == OrgID).Select(m => m.sDatabaseName).FirstOrDefault();
 ...
 new System.Data.SqlClient.SqlConnection(ConfigurationManager.ConnectionStrings["XIDNAAdminContext"].ConnectionString))); //connects to the local server
        ...
        public VMOrganization GetOrganizationDetails(int orgid, string database)
        {
            ...
            var sOrgDB = dbCore.XIAppUsers.Where(m => m.FKiOrgID == org.ID).Select(m => m.sDatabaseName).FirstOrDefault();
            DataContext Spdb = new DataContext(sOrgDB);
        ...
        {
            ModelDbContext dbCore = new ModelDbContext(database);
            var sOrgDB = dbCore.XIAppUsers.Where(m => m.FKiOrgID == model.ConOrganizationID).Select(m => m.sDatabaseName).FirstOrDefault();
            DataContext Spdb = new DataContext(sOrgDB);
        ...
        {
            ModelDbContext dbCore = new ModelDbContext(database);
            var sOrgDB = dbCore.XIAppUsers.Where(m => m.FKiOrgID == model.OrganizationID).Select(m => m.sDatabaseName).FirstOrDefault();
            DataContext Spdb = new DataContext(sOrgDB);
        ...
        {
            ModelDbContext dbCore = new ModelDbContext(database);
            var sOrgDB = dbCore.XIAppUsers.Where(m => m.FKiOrgID == OrganizationID).Select(m => m.sDatabaseName).FirstOrDefault();
            DataContext Spdb = new DataContext(sOrgDB);
        ...
        {
            ModelDbContext dbCore = new ModelDbContext(database);
            var sOrgDB = dbCore.XIAppUsers.Where(m => m.FKiOrgID == LocOrgID).Select(m => m.sDatabaseName).FirstOrDefault();
            DataContext Spdb = new DataContext(sOrgDB);
        ...
        {
            ModelDbContext dbCore = new ModelDbContext(database);
            var sOrgDB = dbCore.XIAppUsers.Where(m => m.FKiOrgID == Source.OrganizationID).Select(m => m.sDatabaseName).FirstOrDefault();
            DataContext Spdb = new DataContext(sOrgDB);
            if (Source.ID == 0)
            {
        ...
        public VMCustomResponse SaveSubscription(VMOrganizationForms model, string database)
        {
            ...
            if (model.ID == 0)
            {
                ...
                {
                    var sOrgDB = dbCore.XIAppUsers.Where(m => m.FKiOrgID == model.SubOrganizationID).Select(m => m.sDatabaseName).FirstOrDefault();
                    DataContext Spdb = new DataContext(sOrgDB);
        ...
        {
            ModelDbContext dbCore = new ModelDbContext(database);
            var sOrgDB = dbCore.XIAppUsers.Where(m => m.FKiOrgID == model.SorFieldOrganizationID).Select(m => m.sDatabaseName).FirstOrDefault();
            DataContext Spdb = new DataContext(sOrgDB);
        ...
        public OrganizationTeams GetUsersList(int OrgID, string database)
        {
            ...
            var Users = dbCore.XIAppUsers.Where(m => m.FKiOrgID == OrgID).ToList();
            ...
            return AllUsers;
            //var Users = dbContext.AspNetGroups.Where(m => m.OrganizationID == FKiOrgID).ToList();
