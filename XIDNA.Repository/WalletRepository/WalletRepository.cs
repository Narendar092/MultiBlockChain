...
namespace XIDNA.Repository
{
    public class WalletRepository : IWalletRepository
    {
        ...
        public DTResponse GetWalletPoliciesList(jQueryDataTableParamModel param, int OrgID, string database)
        {
            ...
            //{
            //    FilteredTabs = Spdb.WalletPolicies.Where(m => m.OrganizationID == FKiOrgID).Where(m => m.Name.Contains(param.sSearch.ToUpper())).ToList();
        ...
        public DTResponse GetRecievedDocuments(string Type, jQueryDataTableParamModel param)
        {
            ...
            if (!string.IsNullOrEmpty(param.sSearch))
            {
                foreach (var items in Orgs)
                {
                    var database = dbcontext.XIAppUsers.Where(m => m.FKiOrgID == items).Select(m => m.sDatabaseName).FirstOrDefault();
                    DataContext Spdb = new DataContext(database);
        ...
        {
            var database = dbcontext.XIAppUsers.Where(m => m.FKiOrgID == model.OrganizationID).Select(m => m.sDatabaseName).FirstOrDefault();
            DataContext Spdb = new DataContext(database);
