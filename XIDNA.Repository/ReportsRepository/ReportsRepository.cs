    ...
    public class ReportsRepository : IReportsRepository
    {
            ...
            if (ColName == "Finance")
            {
                ...
                if (target != null)
                {
                    ...
                    else
                    {
                        ...
                        if (target.IsSMS)
                        {
                            //SendTargetAlert(Convert.ToInt32(TCount), target.Target, UserID, FKiOrgID);
                        }
                        else if (target.IsEmail)
                        {
                            //SendTargetAlertEmail("", FKiOrgID);
                        }
                        else if (target.IsNotification)
                        {
                            //SendTargetAlertNotification(UserID, FKiOrgID);
                        }
        ...
        private void DoSchedulerTask(Schedulers Task, int ReportID, int OrgID, string sDatabase)
        {
            ...
            if (Task.Type == "Email")
            {
                ...
                //    dt = ExportDatatableContent(ReportID, FKiOrgID, Task.UserID);
                ...
                //    Com.SendMail(Task.UserID, FKiOrgID, Attachment, Task.EmailTemplateID);
                //}
                //else
                //{
                //    Com.SendMail(Task.UserID, FKiOrgID, null, Task.EmailTemplateID);
                //}
            }
        ...
        public DataTable ExportDatatableContent(Guid ReportID, int OrgID, int UserID, string sDatabase)
        {
            ModelDbContext dbCore = new ModelDbContext(sDatabase);
            var database = dbCore.XIAppUsers.Where(m => m.FKiOrgID == OrgID).Select(m => m.sDatabaseName).FirstOrDefault();
        ...
        //private void SaveSchedulerLog(int SchID, string Status, string Error, int FKiOrgID)
        //{
        //    DataContext Spdb = new DataContext();
        //    SchedulersLogs Log = new SchedulersLogs();
        //    Log.SchedulerID = SchID;
        //    Log.OrganizationID = FKiOrgID;
        //    Log.LastExecutedOn = DateTime.Now;
        //    Log.Status = Status;
        //    Log.Error = Error;
        //    Spdb.SchedulersLogs.Add(Log);
