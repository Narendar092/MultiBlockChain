...
namespace XIDNA.Repository
{
    public class CommonRepository
    {
        ...
        public List<VMDropDown> GetQuestionsetsDDL(int iUserID, string sOrgName, string sDatabase)
        {
            ...
            QuestionSets = dbContext.QSDefinition.Where(m => m.FKiApplicationID == FKiAppID).Where(m => m.FKiOrgID == iOrgID).ToList().Select(m => new VMDropDown { Value = m.ID, text = m.sName }).ToList();
        ...
        public List<VMDropDown> GetQSFieldsDDL(int iQSStepID, int iUserID, string sOrgName, string sDatabase)
        {
            ...
            Fields = dbContext.XIFieldOrigin.Where(m => m.FKiApplicationID == FKiAppID).Where(m => m.FKiOrgID == iOrgID).Where(m => StepFields.Contains(m.ID)).ToList().Select(m => new VMDropDown { Value = m.ID, text = m.sName }).ToList();
