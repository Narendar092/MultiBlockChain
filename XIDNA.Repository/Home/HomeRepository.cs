...
namespace XIDNA.Repository
{
    public class HomeRepository : IHomeRepository
    {
        ...
        public RightMenuTrees EditRootMenu(int ID, int OrgID, int UserID, string sDatabase)
        {
            ...
            Roles = (from c in dbCore.XIAppRoles.Where(m => m.FKiOrgID == OrgID).ToList()
        ...
        public int DeleteRootMenu(int ID, int OrgID, int UserID, string sDatabase)
        {
            ...
            try
            {
                ...
                //int iOrgID = oMenuList.Select(m => m.FKiOrgID).FirstOrDefault();                
                //var lID = new List<int>();
                //lID = dbContext.RightMenuTrees.Where(m => m.FKiOrgID == iOrgID).Where(m => m.RoleID == iRoleID).Where(m => m.RootName == MenuName).Select(m => m.ID).ToList();
                //for (var i = 0; i < lID.Count(); i++)
                //{
                //    int DelID = lID[i];
        ...
        public List<RightMenuTrees> DeleteNodeDetails(string ParentNode, string NodeID, string ChildrnIDs, string Type, int iRoleID, int UserID, int OrgID, string sDatabase)
        {
            ...
            //AllIDs= dbContext.RightMenuTrees.Where(m => m.RoleID == RoleID).Where(m => m.FKiOrgID == FKiOrgID).Select(m=>m.ID).ToList();
            //Delete Main node.
        ...
        //public RightMenuTrees AddDetailsForMenu(string ParentNode, string NodeID, int FKiOrgID, int UserID, string sDatabase)
        //{
        //    ModeldbContext dbContext = new ModeldbContext();
        //    RightMenuTrees MenuTree = new RightMenuTrees();
        ...
        //              select new VMDropDown { text = c.Name, Value = c.XiLinkID }).ToList();
        //    MenuTree = dbContext.RightMenuTrees.Where(m => m.RoleID == RoleID).Where(m => m.FKiOrgID == FKiOrgID).Where(m => m.MenuID == NodeID).Where(m => m.ParentID == ParentNode).FirstOrDefault();
        //    MenuTree.VMXILink = XILink;
        //    List<VMDropDown> MenuGroup = new List<VMDropDown>();
        //    MenuGroup = (from c in dbContext.MenuGroups
        //                 select new VMDropDown { text = c.sName, Value = c.ID }).ToList();
        ...
        //public List<RightMenuTrees> ShowMenuTreeDetails(int UserID, int FKiOrgID, string sDatabase)
        //{
        //    ModeldbContext dbContext = new ModeldbContext();
        ...
        //    lMenuTree = dbContext.RightMenuTrees.Where(m => m.RoleID == RoleID && m.FKiOrgID == FKiOrgID && m.ParentID == MainID && m.StatusTypeID==10).OrderBy(m => m.Priority).ToList();
        //    return lMenuTree;
        //}
                ...
                else
                {
                    ...
                    if (Oldposition != 0 && Newposition != 0)
                    {
                        if (NoOfPostionsChanged > 0)
                        {
                            ...
                            dbContext.Database.ExecuteSqlCommand("UPDATE [dbo].[RightMenuTrees] SET [Priority] = [Priority] + 1  WHERE [Priority] BETWEEN " + NewPos + " and " + tab.Priority + " and ParentID='" + OldParentID + "' and FKiOrgID= " + OrgID);
                            tab.Priority = tab.Priority - NoOfPostionsChanged;
                        }
                        else
                        {
                            var NewPos = tab.Priority - NoOfPostionsChanged;
                            dbContext.Database.ExecuteSqlCommand("UPDATE [dbo].[RightMenuTrees] SET [Priority] = [Priority] - 1  WHERE [Priority] BETWEEN " + tab.Priority + " and " + NewPos + " and ParentID='" + OldParentID + "' and FKiOrgID= " + OrgID);
                            tab.Priority = tab.Priority - NoOfPostionsChanged;
                        }
                    }
                    if (Oldposition > 0 && Newposition == 0)
        ...
        {
            ModelDbContext dbCore = new ModelDbContext(sDatabase);
            int UserID = dbCore.XIAppUsers.Where(m => m.FKiOrgID == OrgID).Select(m => m.UserID).FirstOrDefault();
            List<VMDropDown> Roles = new List<VMDropDown>();
        ...
        public RightMenuTrees SaveMenuDetails(int iUserID, int RoleID, int OrgID, string RootName, string sDatabase)
        {
            ...
            try
            {
                ...
                if (iRootID == 0)
                {
                    ...
                    //string CreateID = ReplaceSpace + "_" + FKiOrgID + "_" + RoleID;
