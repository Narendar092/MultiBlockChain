    ...
    public class XiLinkRepository : IXiLinkRepository
    {
        ...
        public List<VMQueryPreview> GetTabContent(VMViewPopup Popup, int iUserID, int OrgID, string sDatabase, string sOrgName)
        {
            ...
            foreach (var items in AllTab1Clicks)
            {
                ...
                {
                    Common Com = new Common();
                    var result = Com.GetHeadings(items.FKi1ClickIDXIGUID, sDatabase, OrgID, iUserID, sOrgName);
   ...
   .ToList();
        ...
        public DTResponse GetReportResult(jQueryDataTableParamModel param, VMQuickSearch Search, int iUserID, string sOrgName, string sDatabase, string sCurrentGuestUser, List<cNameValuePairs> nParams)
        {
            ...
            try
            {
                ...
                //string UserIDs = Com.GetSubUsers(Search.UserID, Search.FKiOrgID, sDatabase, sOrgName);
                string Query = oCache.ReplaceExpressionWithCacheValue(NewQuery, nParams);
                //Query = ServiceUtil.ReplaceQueryContent(Query, UserIDs, Search.UserID, Search.FKiOrgID, Search.LeadID);
                Query = ServiceUtil.ReplaceGuestUser(Query, sCurrentGuestUser);
                //Query = ReplaceExpressions(NewQuery, Search.FKiOrgID, Search.LeadID);
        ...
        //public List<RightMenuTrees> GetMenus(List<string> MenuIDs, int FKiOrgID)
        //{
        ...
        //        }
        //        Menus = dbContext.RightMenuTrees.Where(m => IDs.Contains(m.ID) && m.FKiOrgID == FKiOrgID && m.XiLinkID > 0).ToList();
        //    }
        //    else
        //    {
        //        Menus = dbContext.RightMenuTrees.Where(m => m.FKiOrgID == FKiOrgID && m.XiLinkID > 0).ToList();
        //    }            
        //    return Menus;
        //}

        //public List<RightMenuTrees> GetMenus(string MenuName, int FKiOrgID,string sDatabase)
        //{
        //    ModelDbContext dbContext = new ModelDbContext();
        //    List<RightMenuTrees> Menus = new List<RightMenuTrees>();
        ...
        //    //        Names.Add(items);
        //    //    }
        //    //    Menus = dbContext.RightMenuTrees.Where(m => Names.Contains(m.Name) && m.FKiOrgID == FKiOrgID && m.XiLinkID > 0).ToList();
        //    //}
        //    //else
        //    //{
        //    //    Menus = dbContext.RightMenuTrees.Where(m => m.FKiOrgID == FKiOrgID && m.XiLinkID > 0).ToList();
    ...
    .ToList();
        ...
        public VMCustomResponse SaveFiles(int ID, string BOFieldID, int OrgID, List<HttpPostedFileBase> UploadImage, int iUserID, string sOrgName, string sDatabase, string sInstanceID)
        {
                            ...
                            else
                            {
                                ...
                                if (iSavedToDoc == 1)
                                {
                                    ...
                                    if (FileDetails.Preview == "10")
                                    {
                                        ...
                                        //thumb.Save(Path.ChangeExtension(sPath + "\\" + "images_" + FKiOrgID + "_" + ID + "." + sImageFormat, "thumb"));
