...
namespace XICore
{
    public class XIInfraAM4ColumnLineChartComponent
    {
        ...
        public CResult XILoad(List<CNV> oParams)
        {
            ...
            try
            {
                ...
                var Colours = oParams.Where(m => m.sName.ToLower().Contains("sColour".ToLower())).Select(m => m.sValue).FirstOrDefault();
                ...
                if (OneClickID > 0 || (OneClickIDXIGUID != Guid.Empty && OneClickIDXIGUID != null))
                {
                    ...
                    if (sTableResult != null)
                    {
                        foreach (DataRow row in oBOInsdt.Rows)
                        {
                            ...
                            dictionary = Enumerable.Range(0, oBOInsdt.Columns.Count)
                                .ToDictionary(i => oBOInsdt.Columns[i].ColumnName, i => new XIIAttribute
                                {
                    ...
                    oXIGD.sOneClickID = OneClickIDXIGUID.ToString();
                    ...
                    if (!string.IsNullOrEmpty(Colours))
                        oXIGD.sColours = Colours.Split(',').ToList();
                }
