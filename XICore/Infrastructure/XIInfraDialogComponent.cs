...
namespace XICore
{
    public class XIInfraDialogComponent
    {
        ...
        public CResult XILoad(List<CNV> oParams)
        {
            ...
            try
            {
                ...
                {
                    var XIPiDialogID = WrapperParms.Where(m => m.sName == "{-iInstanceID}").Select(m => m.sValue).FirstOrDefault(); // oXIAPI.Get_ParamVal(sSessionID, sGUID, null, Prm); //oParams.Where(m => m.sName.ToLower() == Prm.ToLower()).Select(m => m.sValue).FirstOrDefault();
                    if (XIPiDialogID != null)
                    {
                        DialogID = XIPiDialogID;
                    }
                    else
                    {
                        DialogID = "";
                    ...
                    {
                        int iAppID = 0;
                        if (int.TryParse(sFKiAppID, out iAppID))
                        {
                            iApplicationID = iAppID;
                        }
                    }
                }
                else
                {
                    string sInstanceID = oParams.Where(m => m.sName == "DialogID").Select(m => m.sValue).FirstOrDefault();
                ...
                XIDDialog cXIDia = new XIDDialog();
                XIDXI oDXI = new XIDXI();
                oDXI.FKiApplicationID = iApplicationID;
                var oXiDialogDef = oDXI.Get_DialogDefinition(DialogID);
                if (oXiDialogDef.bOK && oXiDialogDef.oResult != null)
                {
                    cXIDia = (XIDDialog)oXiDialogDef.oResult;
                }
                cXIDia.ddlApplications = oConfig.Get_AllApplications();
                oCResult.oResult = cXIDia;
