...
namespace XICore
{
    ...
    public class XIInfraRoles
    {
        ...
        public Guid iParentIDXIGUID { get; set; }
        ...
        public List<XIInfraRoles> CreateHierarchy(List<XIInfraRoles> parentRoles, List<XIInfraRoles> allRoles) // Currently using to create roles as tree object. Role hierarchy is not in use.
        {
            try
            {
                parentRoles.ForEach(pr =>
                {
                    List<XIInfraRoles> subGroups = allRoles.Where(m => m.iParentIDXIGUID == pr.XIGUID).ToList();
                    if(subGroups.Count > 0)
                    {
                        CreateHierarchy(subGroups, allRoles);
                        pr.SubGroups = subGroups;
                    }
                });
                return parentRoles;
            }
        ...
        public string GetRoleTheme(string sCoreDatabase)
        {
            ...
            UserParams["XIGUID"] = iThemeIDXIGUID;
        ...
        public CResult GetAllRolesList(string sCoreDatabase, int? iOrgID)
        {
            CResult oCResult = new CResult(); // always
            CResult oCR = new CResult(); // always
            long iTraceLevel = 10;
