...
namespace XICore
{
    public class XIInfraAM4BarChartComponent
    {
        ...
        public CResult XILoad(List<CNV> oParams)
        {
            ...
            try
            {
                ...
                var sLegendPosition = oParams.Where(m => m.sName.ToLower() == "sLegendPosition".ToLower()).Select(m => m.sValue).FirstOrDefault();
                ...
                if (OneClickID > 0 || (OneClickIDXIGUID != Guid.Empty && OneClickIDXIGUID != null))
                {
                    ...
                    oXIGD.sLegendPosition = sLegendPosition;
