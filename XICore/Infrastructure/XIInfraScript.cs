...
namespace XICore
{
    public class XIInfraScript
    {
        ...
        public CResult XIScripting(string sScriptID, string sGUID, string iInstanceID, int iUserID, string sDatabase, int iCustomerID, string ProductName, string Version, string ProductCode, string dtOldCoverStart, string rOldGrossPremium, int QuoteID, string sSessionID, int iOrgID=0, string ProductID= null)
        {
            ...
            try
            {
                ...
                if (!Guid.TryParse(sScriptID, out gScriptIDXIGUID))
                {
                    int.TryParse(sScriptID, out iScriptID);

                }
                var oResult = RunXIScript(iInstanceID, sGUID, sScriptID, sDatabase, iUserID, iCustomerID, ProductName, Version, ProductCode, dtOldCoverStart, rOldGrossPremium, QuoteID, sSessionID, iOrgID, ProductID);
                if (oResult.xiStatus == xiEnumSystem.xiFuncResult.xiSuccess && oResult.oResult != null)
                {
                    ...
                    {
                        sScriptResult.FKiScriptIDXIGUID = gScriptIDXIGUID;
                    }
                    ...
                    if (sUserError != null && sUserError.Contains('{') && sUserError.Contains('}'))
                    {
        ...
        private CResult RunXIScript(string ID, string sGUID, string iScriptID, string sDatabase, int iUserID, int iCustomerID, string ProductName, string Version, string ProductCode, string dtOldCoverStart, string rOldGrossPremium, int QuoteID, string sSessionID, int iOrgID, string ProductID)
        {
            CResult oCResult = new CResult();
            try
            {
                ...
                CNV OrgID = new CNV();
                OrgID.sName = "iOrgID";
                OrgID.sValue = iOrgID.ToString();
                lParam.Add(OrgID);
                CNV iProductID = new CNV();
                iProductID.sName = "ProductID";
                iProductID.sValue = ProductID.ToString();
                lParam.Add(iProductID);
                oCResult.oTraceStack.Add(new CNV { sName = "Stage", sValue = "Name Value pairs added successfully" });
                XIIXI oXI = new XIIXI();
                var oBOI = oXI.BOI("XIBOScript_T", iScriptID); 
                object oResult = null;
                if (oBOI != null) { 
...
using System.Diagnostics;
               ...
               ";
                ...
                loParameters.ReferencedAssemblies.Add(sdllPath + "\\RestSharp.dll");
        ...
        public CResult RunScript(List<CNV> oParams, iSiganlR oSignalR)
        {
            CResult oCResult = new CResult();
            CResult oCR = new CResult();
            ...
            {
                XIInfraCache oCache = new XIInfraCache();
                var iBODID = oParams.Where(m => m.sName.ToLower() == "iBODID".ToLower()).Select(m => m.sValue).FirstOrDefault();
                var iBOIID = oParams.Where(m => m.sName.ToLower() == "iBOIID".ToLower()).Select(m => m.sValue).FirstOrDefault();
                ...
                MethodInfo methodInfoFromCache = (MethodInfo)oCache.GetFromCache("SingleC#Compiler_" + ScriptID);
                List<CNV> lParam = new List<CNV>();
                lParam.Add(new CNV { sName = "iBODID", sValue = iBODID });
                lParam.Add(new CNV { sName = "iBOIID", sValue = iBOIID });
                if (methodInfoFromCache != null)
                {
                    oResult = methodInfoFromCache.Invoke(null, new object[] { lParam, oSignalR });
                }
                else
                {
                    ...
                    if (info.xiStatus == xiEnumSystem.xiFuncResult.xiSuccess && info.oResult != null)
                    {
                        ...
                        methodInfoFromCache = methodInfo;
                        oResult = methodInfoFromCache.Invoke(null, new object[] { lParam, oSignalR });
                        oCache.InsertIntoCache(methodInfoFromCache, "SingleC#Compiler_" + ScriptID);
                    }
                }
