...
namespace XICore
{
    public class XIInfraLayoutDetailsComponent
    {
        ...
        public CResult XILoad(List<CNV> oParams)
        {
            ...
            try
            {
                ...
                {
                    var XiPlaceHolderID = WrapperParms.Where(m => m.sName == "{-iInstanceID}").Select(m => m.sValue).FirstOrDefault(); // oXIAPI.Get_ParamVal(sSessionID, sGUID, null, Prm); //oParams.Where(m => m.sName.ToLower() == Prm.ToLower()).Select(m => m.sValue).FirstOrDefault();
                    if (XiPlaceHolderID != null)
                    {
                        sPlaceHolderID = XiPlaceHolderID;
                    }
                    else
                    {
                        sPlaceHolderID = "";
                ...
                {
                    string sInstanceID = oParams.Where(m => m.sName == "PlaceHolderID").Select(m => m.sValue).FirstOrDefault();
                    ...
                    {
                        sPlaceHolderID = sInstanceID;
                        //if (long.TryParse(sInstanceID, out iPlaceHolderID))
                        //{
