...
namespace XICore
{
    public class CCodeLine
    {
        ...
        private string sMyAOperator = "";

        private string sMySubOperator = "";
        ...
        string sADelim = mGlobals.sADelim;
        string sSubDelim = mGlobals.sSubDelim;
        public CResult DeSerialiseMe(string sLine, string sOriginalLine, string sFlag = null)
        {
            ...
            int iAOperator = 0;
            int iSubOperator = 0;
            string sRemainingLine = string.Empty;
            ...
            try
            {
                ...
                iLastBracket = sMyLine.LastIndexOf(sCB, sMyLine.Length) + 1; //InStrRev(sMyLine, sCB);
                if (iFirstBracket != 0 && iLastBracket != 0)
                {
                    // so there is a function to resolve here
                    bSubFunction = true;
                }
                else if (iFirstBracket != 0 || iLastBracket != 0)
                {
                    ...
                    oMyResult.sMessage = oMyResult.sMessage + "\r\n" + "Bracket Mismatch: First Bracket position = "
                                + iFirstBracket + ", Last Bracket position = " + iLastBracket + " [Line = \'" + sMyLine + "\']";
                }
                else
                {
                    // no more brackets
                    if (sFlag == "algorithm")
                    {
                        if (sMyLine.StartsWith("ag") || sMyLine.StartsWith("m"))
                        {
                            bSubFunction = true;
                        }
                        else if (sMyLine.Contains(':') && !sMyLine.StartsWith("-"))
                        {
                            bSubFunction = true;
                        }
                        else if (sMyLine.Contains('|'))
                        {
                            bSubFunction = true;
                        }
                        else if (sMyLine.Contains('_'))
                        {
                            bSubFunction = true;
                        }
                        else
                ...
                if (sFlag == "algorithm")
                {
                    iSubOperator = sMyLine.IndexOf(sSubDelim) + 1;
                    iAOperator = sMyLine.IndexOf(sADelim) + 1;
                }
                iOperator = sMyLine.IndexOf(sFDelim) + 1;
                if (iOperator == 0 && iAOperator == 0 && iSubOperator == 0)
                {
                ...
                else
                {
                    if (sFlag == "algorithm")
                    {
                        if (iAOperator > 0)
                        {
                            sAOperator = sMyLine.Substring(0, (iAOperator - 1));
                            sRemainingLine = sMyLine.Substring(iAOperator, (sMyLine.Length - iAOperator));
                            sRemainingOrig = sMyLineOrig.Substring(iAOperator, (sMyLineOrig.Length - iAOperator));
                        }
                        //else if (iSubOperator > 0)
                        //{
                        //    sSubOperator = sMyLine.Substring(0, (iSubOperator - 1));
                        //    sRemainingLine = sMyLine.Substring(iSubOperator, (sMyLine.Length - iSubOperator));
                        //    sRemainingOrig = sMyLineOrig.Substring(iSubOperator, (sMyLineOrig.Length - iSubOperator));
                        //}
                        else
                        {
                            if (iOperator != 0)
                    ...
                    if (bSubFunction)
                    {
                        ...
                        for (j = 0; j <= (sRemainingLine.Length - 1); j++)
                        {
                            ...
                            if (sChar == sParamDelim)
                            {
                                if (iBracketLevel == 0)
                                {
                                    ...
                                    this.AddParam(sCurrentParam, sCurrentOrig, sFlag);
                                    sCurrentParam = "";
                                    sCurrentOrig = "";
                                }
                                else
                            ...
                            else if (sFlag == "algorithm" && sChar == sADelim)
                            {
                                if (iBracketLevel == 0)
                                {
                                    sCurrentParam = sCurrentParam.Substring(0, (sCurrentParam.Length - 1));
                            ...
                            //else if (sFlag == "algorithm" && j > 1 && sChar == sSubDelim)
                            //{
                            //    if (iBracketLevel == 0)
                            //    {
                            //        sCurrentParam = sCurrentParam.Substring(0, (sCurrentParam.Length - 1));
                            ...
                            //    }
                            //}
                            else if (sFlag == "algorithm" && sChar == sFDelim)
                            {
                                if (iBracketLevel == 0)
                                {
                                    sCurrentParam = sCurrentParam.Substring(0, (sCurrentParam.Length - 1));
                            ...
                            {
                                //if (sFlag == "algorithm" && sRemainingLine.Contains('_'))
                                //{
                                //    sRemainingLine = sRemainingLine.Replace(sChar, "");
                                //}
                                //else
                                //{
                            ...
                            if ((j + 1) == sRemainingLine.Length && sChar != sParamDelim)
                            {
            ...
            catch (Exception ex)
            {
                ...
                oMyResult.xiStatus = xiEnumSystem.xiFuncResult.xiError;
                oMyResult.sMessage = oMyResult.sMessage + "\r\n" + "Bracket Mismatch: [Line = \'" + sMyLine + "\']";
            }
            return oMyResult;
        }

        public CResult AddParam(string sParameter, string sOriginalParam, string sFlag = null)
        {
            CResult oMyResult = new CResult();
            ...
            try
            {
                ...
                oSubCodeLine.DeSerialiseMe(sParameter, sOriginalParam, sFlag);
        ...
        public CResult ParsedLine(long iLevel)
        {
            ...
            try
            {
                ...
                if (iParamCount > 0)
                {
                    sConcat = sTabs + sOperator + " " + sOB + "\r\n";
                    foreach (var oLine in NCodeLines.Values)
                    {
                        sConcat = sConcat + oLine.ParsedLine(iLevel + 1).oResult + "\r\n";
                    }
                    sConcat = sConcat + "\r\n" + sTabs + sCB;
                }
                else
        ...
        public CResult FormattedLine(long iLevel)
        {
            ...
            try
            {
                ...
                if (iParamCount > 0)
                {
                    sConcat = sTabs + this.OperatorFormat(sOperator) + " " + "\r\n";
                    foreach (var oLine in NCodeLines.Values)
                    {
                        sConcat = sConcat + oLine.FormattedLine(iLevel + 1).oResult + "\r\n";
                        k = k + 1;
                        if (k < NCodeLines.Count)
                        {
                            sConcat = sConcat + sTabs + this.OperatorGlyph(sOperator);
                        }
                    }
                    sConcat = sConcat + "\r\n";
                }
            ...
            try
            {
                ...
                if (iParamCount > 0)
                {
                    foreach (var oCParameter in NCodeLines.Values)
                    {
                        ...
                        {
                            oMyResult.xiStatus = xiEnumSystem.xiFuncResult.xiError;
                            oMyResult.sMessage = oMyResult.sMessage + oCR.sMessage + "\r\n";
                        }
                        else
                        {
                ...
                if (oMyResult.xiStatus != xiEnumSystem.xiFuncResult.xiError)
                {
                    ...
                    else if (sOperator == "" && oLastChildResult != null)
                    {
            ...
            try
            {
                ...
                if (sOperator != "")
                {
                    ...
                    switch (sOperator)
                    {
                        ...
                        case "increment":
                        case "decrement":
                            if (iParamCount != 2 || NCodeLines.Count != 2)
                            {
                                oMyResult.xiStatus = xiEnumSystem.xiFuncResult.xiError;
                                oMyResult.sMessage = "Param count for function \'" + sOperator + "\' is "
                                            + iParamCount + " when it should be 2 [Line=\'" + sMyLine + "\']";
                            }
                            else
                            {
                                switch (sOperator)
                        ...
                        case "||":
                        case "contains":
                            if ((iParamCount != 2 || NCodeLines.Count != 2) && (iParamCount != 3 || NCodeLines.Count != 3))
                            {
                                oMyResult.xiStatus = xiEnumSystem.xiFuncResult.xiError;
                                oMyResult.sMessage = "Param count for condition \'" + sOperator + "\' is "
                                            + iParamCount + " when it should be 2 [Line=\'" + sMyLine + "\']";
                            }
                            else
                            {
                                switch (sOperator)
                                {
                                    ...
                                    case ">":
                                        ...
                                        else if (Utility.IsDateTime(NCodeLines["CL1"].sResult) && Utility.IsDateTime(NCodeLines["CL2"].sResult))
                                        {
                                            if (Utility.ConvertToDate(NCodeLines["CL1"].sResult) > Utility.ConvertToDate(NCodeLines["CL2"].sResult))
                                            {
                                                sMyResult = "true";
                                            }
                                            else
                                    ...
                                    case "or":
                                    case "||":
                                        if (Utility.IsNumeric(NCodeLines["CL1"].sResult) && (Utility.IsNumeric(NCodeLines["CL2"].sResult) && Utility.IsNumeric(NCodeLines["CL3"].sResult)))
                                        {
                                            if (double.Parse(NCodeLines["CL1"].sResult) == double.Parse(NCodeLines["CL2"].sResult))
                                            {
                                                sMyResult = "true";
                                    ...
                                    case "contains":
                                        ...
                                        {
                                            if (NCodeLines["CL1"].sResult.Contains(@"\|\|\|") || NCodeLines["CL1"].sResult.Contains("|||"))
                                            {
                                            ...
                                            else if (NCodeLines["CL2"].sResult.Contains(@"\|\|\|") || NCodeLines["CL2"].sResult.Contains("|||"))
                                            {
                                                string[] STRarray = Regex.Split(NCodeLines["CL2"].sResult, @"\|\|\|");
                                                if (STRarray.Contains(NCodeLines["CL1"].sResult))
                                                {
                                                    sMyResult = "true";
                                                }
                                                else
                                            ...
                                            {
                                                if (NCodeLines["CL1"].sResult.Contains(NCodeLines["CL2"].sResult.Replace(" ", "")))
                                                    sMyResult = "true";
                                                else
                                                    sMyResult = "false";
                                            }
                        ...
                        case "if":
                            ...
                            else
                            {
                                ...
                                {
                                    bResult = oCR.oResult == null ? false : (bool)oCR.oResult;
                                    if (bResult == true)
                                    {
                                        sMyResult = sIf1;
                                    }
                        ...
                        case "c":
                            ...
                            break;
                        case "lower":
                            if (iParamCount != 1 || NCodeLines.Count != 1)
                            {
                                oMyResult.xiStatus = xiEnumSystem.xiFuncResult.xiError;
                                oMyResult.sMessage = "Param count for function \'" + sOperator + "\' is "
                                            + iParamCount + " when it should be 1 [Line=\'" + sMyLine + "\']";
                            }
                            else
                            {
                                try
                                {
                                    if (!string.IsNullOrEmpty(NCodeLines["CL1"].sResult))
                                    {
                                        sMyResult = NCodeLines["CL1"].sResult.ToLower();
                                    }
                                    else
                                    {
                                        sMyResult = "";
                        ...
                        case "upper":
                            ...
                            else
                            {
                                try
                                {
                                    if (!string.IsNullOrEmpty(NCodeLines["CL1"].sResult))
                                    {
                                        sMyResult = NCodeLines["CL1"].sResult.ToUpper();
                                    }
                                    else
                                    {
                                        sMyResult = "";
                        ...
                        case "match":
                            if (iParamCount < 2 || NCodeLines.Count < 2)
                            {
                                oMyResult.xiStatus = xiEnumSystem.xiFuncResult.xiError;
                                oMyResult.sMessage = "Param count for function \'" + sOperator + "\' is "
                                            + iParamCount + " when it should be greater than 1 [Line=\'" + sMyLine + "\']";
                            }
                            else
                            {
                                try
                        ...
                        case "n":
                            ...
                            break;
                        case "sum":
                            if (NCodeLines.Count > 0)
                            {
                                double rCount = 0;
                                foreach (var oParam in NCodeLines.Values)
                                {
                                    rCount += double.Parse(oParam.sResult);
                                    sMyResult = rCount.ToString();
                                }
                            }
                        ...
                        case "replace":
                        case "rp":
                            if (iParamCount != 3 || NCodeLines.Count != 2)
                            {
                                oMyResult.xiStatus = xiEnumSystem.xiFuncResult.xiError;
                                oMyResult.sMessage = "Param count for function \'" + sOperator + "\' is "
                                            + iParamCount + " when it should be 3 [Line=\'" + sMyLine + "\']";
                            }
                            else
                            {
                                try
                        ...
                        case "xi.invoke":
                        case "xi.secset":
                        case "xi.bindsignalrdata":
                        case "xi.oneclick":
                        case "xi.oneclickcount":
                            if (oMyXIAPI == null)
                            {
                                oMyXIAPI = new CXIAPI();
                            }
                            switch (sOperator)
                            {
                                case "xi.p":
                                    ...
                                    else
                                    {
                                        try
                                        {
                                            ...
                                            if (oCR != null)
                                            {
                                                if (oCR.xiStatus == xiEnumSystem.xiFuncResult.xiSuccess)
                                                {
                                                    sMyResult = oCR.oResult == null ? null : oCR.oResult.ToString();
                                                }
                                                else
                                                {
                                    ...
                                    break;
                                case "xi.qsxivalue":
                                    if (iParamCount < 3 || NCodeLines.Count < 3)
                                    {
                                        oMyResult.xiStatus = xiEnumSystem.xiFuncResult.xiError;
                                        oMyResult.sMessage = "Param count for function \'" + sOperator + "\' is "
                                                    + iParamCount + " when it should be 1 [Line=\'" + sMyLine + "\']";
                                        ...
                                        try
                                        {
                                            ...
                                            switch (sOperator)
                                            {
                                                case "xi.a":
                                                    if (iParamCount == 3)
                                                    {
                                                        oCR = oMyXIAPI.Attr(NCodeLines["CL1"].sResult, NCodeLines["CL2"].sResult, NCodeLines["CL3"].sResult,"","","",sSessionID,sGUID);
                                                    }
                                                    else if (iParamCount == 4)
                                                    {
                                                        oCR = oMyXIAPI.Attr(NCodeLines["CL1"].sResult, NCodeLines["CL2"].sResult, NCodeLines["CL3"].sResult, NCodeLines["CL4"].sResult,"","",sSessionID,sGUID);
                                                    }
                                                    else if (iParamCount == 5)
                                                    {
                                                        oCR = oMyXIAPI.Attr(NCodeLines["CL1"].sResult, NCodeLines["CL2"].sResult, NCodeLines["CL3"].sResult, NCodeLines["CL4"].sResult, NCodeLines["CL5"].sResult,"",sSessionID,sGUID);
                                                    }
                                                    else if (iParamCount == 6)
                                                    {
                                                        oCR = oMyXIAPI.Attr(NCodeLines["CL1"].sResult, NCodeLines["CL2"].sResult, NCodeLines["CL3"].sResult, NCodeLines["CL4"].sResult, NCodeLines["CL5"].sResult, NCodeLines["CL6"].sResult,sSessionID,sGUID);
                                                    }
                                                    break;
                                                case "xi.a.f":
                                ...
                                case "xi.count":
                                    ...
                                    else
                                    {
                                        try
                                        {
                                            ...
                                            if (iParamCount == 2)
                                            {
                                                oCR = oMyXIAPI.Count(NCodeLines["CL1"].sResult, NCodeLines["CL2"].sResult);
                                            }
                                            else if (iParamCount == 3)
                                            {
                                                oCR = oMyXIAPI.Count(NCodeLines["CL1"].sResult, NCodeLines["CL2"].sResult, NCodeLines["CL3"].sResult);
                                            }
                                            if (oCR != null)
                                            {
                                                if (oCR.xiStatus == xiEnumSystem.xiFuncResult.xiSuccess)
                                    ...
                                    break;
                                case "xi.a.fk":
                                    if (iParamCount != 4 || NCodeLines.Count != 4)
                                    {
                                        oMyResult.xiStatus = xiEnumSystem.xiFuncResult.xiError;
                                        oMyResult.sMessage = "Param count for function \'" + sOperator + "\' is "
                                                    + iParamCount + " when it should be 4 [Line=\'" + sMyLine + "\']";
                                    }
                                    else
                                    {
                                        try
                                ...
                                case "xi.insertbo":
                                    ...
                                    else
                                    {
                                        try
                                        {
                                            ...
                                            if (iParamCount == 4)
                                            {
                                                oCR = oMyXIAPI.InsertBO(NCodeLines["CL1"].sResult, NCodeLines["CL2"].sResult, NCodeLines["CL3"].sResult, NCodeLines["CL4"].sResult, sGUID, sSessionID);
                                            }
                                            if (oCR != null)
                                            {
                                                if (oCR.xiStatus == xiEnumSystem.xiFuncResult.xiSuccess)
                                ...
                                case "xi.secset":
                                    ...
                                    else
                                    {
                                        try
                                        {
                                            ...
                                            if (iParamCount == 1)
                                            {
                                                oCR = oMyXIAPI.GetSettingConfig(NCodeLines["CL1"].sResult, "", sGUID, sSessionID);
                                            }
                                            else if (iParamCount == 2)
                                            {
                                                oCR = oMyXIAPI.GetSettingConfig(NCodeLines["CL1"].sResult, NCodeLines["CL2"].sResult, sGUID, sSessionID);
                                            }
                                            if (oCR != null)
                                            {
                                                if (oCR.xiStatus == xiEnumSystem.xiFuncResult.xiSuccess)
                                ...
                                case "xi.invoke":
                                    ...
                                    else
                                    {
                                        try
                                        {
                                            ...
                                            if (iParamCount == 4)
                                            {
                                                oCR = oMyXIAPI.Invoke(NCodeLines["CL1"].sResult, NCodeLines["CL2"].sResult, NCodeLines["CL3"].sResult, NCodeLines["CL4"].sResult, sGUID, sSessionID);
                                            }
                                            if (oCR != null)
                                            {
                                                if (oCR.xiStatus == xiEnumSystem.xiFuncResult.xiSuccess)
                                ...
                                case "xi.bindsignalrdata":
                                    ...
                                    else
                                    {
                                        try
                                        {
                                            ...
                                            if (iParamCount == 3)
                                            {
                                                oCR = oMyXIAPI.BindSignalRData(NCodeLines["CL1"].sResult, NCodeLines["CL2"].sResult, NCodeLines["CL3"].sResult, sGUID, sSessionID);
                                            }
                                            if (oCR != null)
                                            {
                                                if (oCR.xiStatus == xiEnumSystem.xiFuncResult.xiSuccess)
                                ...
                                case "xi.oneclick":
                                    ...
                                    else
                                    {
                                        try
                                        {
                                            ...
                                            if (iParamCount == 2)
                                            {
                                                oCR = oMyXIAPI.ExecuteOneclick(NCodeLines["CL1"].sResult, NCodeLines["CL2"].sResult, sGUID, sSessionID);
                                            }
                                            if (oCR != null)
                                            {
                                                if (oCR.xiStatus == xiEnumSystem.xiFuncResult.xiSuccess)
                                        ...
                                        try
                                        {
                                            ...
                                            if (oCR != null)
                                            {
                                                if (oCR.xiStatus == xiEnumSystem.xiFuncResult.xiSuccess)
                                                {
                                                    if(oCR.oResult != null)
                                                    {
                                                        var BoList = (Dictionary<string, XIIBO>)oCR.oResult;
                                                        oCR.oResult=BoList.Count;
                                                    }
                                                    sMyResult = oCR.oResult == null ? "0" : oCR.oResult.ToString();
                                                }
                                                else
                                                {
                                                }
                            ...
                            else
                            {
                                try
                                {
                                    if (oMyAlgoI != null)
                                    {
                                        ...
                                        sMyKey = NCodeLines["CL1"].sResult.ToString().ToLower();
                                        CMethodStepInstance oMethodStepInstance = new CMethodStepInstance();
                                        if (oMyAlgoI.oSteps.ContainsKey(sMyKey))
                                        {
                                            oMethodStepInstance = oMyAlgoI.oSteps[sMyKey];
                                            if (oMethodStepInstance.oMethodResult != null)
                                            {
                                                sMyResult = oMethodStepInstance.oMethodResult.ToString().ToLower();
                                            }
                                        }

                                        //CMethodStepInstance oMethodStepInstance;
                                        //oMethodStepInstance = oMyAlgoI.oSteps[sMyKey];
                                        //oCR = oMethodStepInstance.Get_ParameterObject(NCodeLines["CL1"].sResult);
                                        //if (oCR.xiStatus == xiEnumSystem.xiFuncResult.xiSuccess)
                                        //{
                                        //    sMyResult = oCR.oResult == null ? null : oCR.oResult.ToString();
                                        //}
                        ...
                        case "trim":
                        case "t":
                            if (!string.IsNullOrEmpty(NCodeLines["CL1"].sResult) && !string.IsNullOrEmpty(NCodeLines["CL2"].sResult))
                            {
                                int iLength = NCodeLines["CL2"].sResult.Length;
                                if (iLength == 1)
                                {
                                    sMyResult = NCodeLines["CL1"].sResult.Trim(char.Parse(NCodeLines["CL2"].sResult));
                                }
                                else if (iLength == 2)
                                {
                                    if (NCodeLines["CL1"].sResult.StartsWith("0"))
                                        sMyResult = NCodeLines["CL1"].sResult.Substring(2);
                                    else
                                        sMyResult = NCodeLines["CL1"].sResult.Trim(char.Parse(NCodeLines["CL2"].sResult.Substring(1)));
                                }
                            }
                            else
                            {
                                sMyResult = NCodeLines["CL1"].sResult;
                            }
                        ...
                        case "xi.function":
                            if (!string.IsNullOrEmpty(NCodeLines["CL1"].sResult))
                            {
                                ...
                                Type Ltype;
                                object objclass;
                                exceutable = Assembly.Load("XICore");
                                Ltype = exceutable.GetType("XICore" + "." + "XIFunctions");
                                objclass = Activator.CreateInstance(Ltype);
                                MethodInfo method = Ltype.GetMethod(NCodeLines["CL1"].sResult);
                                object Response = new object();
                                if (NCodeLines.ContainsKey("CL4"))
                                {
                                    object[] parametersArray = new object[] { NCodeLines["CL2"].sResult, NCodeLines["CL3"].sResult, NCodeLines["CL4"].sResult };
                                    Response = (object)method.Invoke(objclass, parametersArray);
                                }
                                else
                                {
                                    object[] parametersArray = new object[] { NCodeLines["CL2"].sResult, NCodeLines["CL3"].sResult };
                                    Response = (object)method.Invoke(objclass, parametersArray);
                                }
                                
                                oMyResult.oResult = Response;
                                sMyResult = Response.ToString();
                ...
                else
                {
                    ...
                    {
                        if (sCleanedValue.Substring(0, 1) == "\'" || sCleanedValue.Substring(0, 1) == "\"")
                        {
        ...
        private string RT_Resolve(string sValue)
        {
            ...
            {
                sResolved = sValue;
                if (!string.IsNullOrEmpty(sValue)&&( sValue.Contains("xim.") || sValue.Contains("xis.")))
                {
                    if (!(oMyAlgoI == null))
                    {
                        // TO DO - get access to the same lookup used by method line instances. Actually this must have a method line
                        ...
                        if (oCR.xiStatus == xiEnumSystem.xiFuncResult.xiSuccess)
                        {
                            sResolved = oCR.oResult == null ? null : oCR.oResult.ToString();
        ...
        public string sSubOperator
        {
            get
            {
                return sMySubOperator;
            }
            set
            {
                sMySubOperator = value;
            }
        }
        ...
        public CResult Get_BooleanResult(string sValue)
        {
            ...
            try
            {
                ...
                if (sValLower == "y" || sValLower == "true")
                {
                    oMyResult.xiStatus = xiEnumSystem.xiFuncResult.xiSuccess;
                    oMyResult.oResult = true;
                }
                else if (sValLower == "n" || sValLower == "false")
                {
                    oMyResult.xiStatus = xiEnumSystem.xiFuncResult.xiSuccess;
            ...
            {
                oMyResult.xiStatus = xiEnumSystem.xiFuncResult.xiError;
                oMyResult.sMessage = "ERROR: [" + Get_Class() + "." + System.Reflection.MethodBase.GetCurrentMethod().Name + "] - "
        ...
        public string SerialiseNodeFromXML(System.Xml.XmlNode oXMLNode, CCodeLine oCLine, long iUseKey)
        {
            CCodeLine oCNewNode;
            // Warning!!! Optional parameters not supported
            string sClass = "";
            ...
            if (oXMLNode.NodeType == System.Xml.XmlNodeType.XmlDeclaration && oCLine == null)
            {
                return "";
            }
            //  ----- Add the node itself.
