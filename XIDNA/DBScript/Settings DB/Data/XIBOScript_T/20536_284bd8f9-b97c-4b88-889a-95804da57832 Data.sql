INSERT INTO [XIBOScript_T] ([ID],
 ...
 [FKiTagSpaceIDXIGUID],
 [FKiVersionIDXIGUID]) Values(N'20536',
 N'296',
 N'0',
 NULL,
 N'Charity Product3',
  ...
  public CResult Calculation(string sGUID, string iInstanceID, int iUserID, int iCustomerID, string sDataBase, string sProductName, string sVersion, string sProductCode, int iQuoteID, string sSessionID = null) {
    ...
    try {
      ...
      if (sSessionID == null) {
        ...
        /*var oLIst = oIXI.BOI("QS Instance", iInstanceID.ToString());          oQSI = oLIst.Structure("charity qs").XILoad();   */
        var oLIst = oIXI.BOI("Aggregations", iQuoteID.ToString());
        oQSI = oLIst.Structure("Quotes XML").XILoad();
        /*oLIst.Structure("charity qs").XILoad();    */
      }
      /*var ostructureInstance1 = oQSI.oStructureInstance.FirstOrDefault().Value.FirstOrDefault();    */
      var ostructureInstance = oQSI.oStructureInstance.FirstOrDefault().Value.FirstOrDefault().SubChildI.Where(x => x.Key == "QS Instance1").Select(x => x.Value).FirstOrDefault().FirstOrDefault(); /*var ostructureInstance = ostructureInstance2.FirstOrDefault();  */
      ...
      oBOI.Attributes["XIGUID"] = new XIIAttribute {
        sName = "XIGUID",
          sValue = oCurrentQuoteI.Attributes["XIGUID"].sValue,
          bDirty = true
      };
      ...
      oBOI.Attributes["rAdmin"] = new XIIAttribute {
        sName = "rAdmin",
          sValue = String.Format("{0:0.00}", (oProductI.Attributes["iAdminType"].sValue == "10") ? (rFinalQuote * Convert.ToDouble(oProductI.Attributes["zDefaultAdmin"].sValue) / 100) : Convert.ToDouble(oProductI.Attributes["zDefaultAdmin"].sValue)),
          bDirty = true
      };
      oBOI.Attributes["rPaymentToInsurer"] = new XIIAttribute {
        sName = "rPaymentToInsurer",
          sValue = String.Format("{0:0.00}", rFinalQuote - (Convert.ToDouble(oBOI.Attributes["rCommission"].sValue) + Convert.ToDouble(oBOI.Attributes["rAdmin"].sValue))),
          bDirty = true
      };
      oBOI.Attributes["zCommissionOverride"] = new XIIAttribute {
        sName = "zCommissionOverride",
          sValue = String.Format("{0:0.00}", oProductI.Attributes["zCommission"].sValue),
          bDirty = true
      };
      oBOI.Attributes["rOverrideAdmin"] = new XIIAttribute {
        sName = "rOverrideAdmin",
          sValue = String.Format("{0:0.00}", oProductI.Attributes["zDefaultAdmin"].sValue),
          bDirty = true
      };
      ...
      {
          sName = "sInsurer",
          sValue = supplierloading.Attributes["sName"].sValue,
          bDirty = true
      };*/
      ...
      if (oBOIList.Count > 0) {
        ...
        /* xibulk.SaveBulk(dtbulk, oBOIList[0].BOD.iDataSourceoBOIList[0].BOD.iDataSourceXIGUID.ToString(), oBOIList[0].BOD.TableName);*/
