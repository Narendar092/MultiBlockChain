Delete XIBOScript_T where XIGUID='5f49cf72-17c3-4cab-9efe-691adf72c61c'
INSERT INTO [XIBOScript_T] ([ID],
 ...
 [FKiTagSpaceIDXIGUID],
 [FKiVersionIDXIGUID]) Values(N'30557',
 N'296',
 N'0',
 NULL,
 N'Adrian Flux',
...
public class PolicyCalculation
{
    public CResult Calculation(string iInsatnceID, int iUserID, int iCustomerID, string sVersion, string sProductCode, int iQuoteID)
    {
        ...
        Info.Add("Adrian Flux script running");
        XIInstanceBase oIB = new XIInstanceBase();
        CResult oresult = new CResult();
        ...
        try
        {
            ...
            if (ostructureInstance.XIIValues.Count() > 0)
            {
                ...
                if (ostructureInstance.XIIValues.ContainsKey("sCanyouconfirmwhoistheregisteredkeeper"))
                {
                    var registeredkeeper = ostructureInstance.XIIValues["sCanyouconfirmwhoistheregisteredkeeper"].sResolvedValue;
                    if (registeredkeeper == "Proposer" || registeredkeeper == "Named Driver")
                    {
                        registeredkeepers = true;
                    }
                    else
                    {
                        bProductValidation = false;
                        var Steps = BaseCal.PageValidation(ostructureInstance.XIIValues["sCanyouconfirmwhoistheregisteredkeeper"].sDisplayName);
                        bool bCurrentStep = true;
                        if (Steps.xiStatus == 0)
                        {
                            string sBase = "";
                ...
                if (ostructureInstance.XIIValues.ContainsKey("Have you been quoted by Adrian Flux?"))
                {
                    int quoted = Convert.ToInt32(ostructureInstance.XIIValues["Have you been quoted by Adrian Flux?"].sValue);
                    if (quoted == 20)
                    {
                        quotes = true;
                    }
                    ...
                    {
                        bProductValidation = false;
                        var Steps = BaseCal.PageValidation(ostructureInstance.XIIValues["Have you been quoted by Adrian Flux?"].sDisplayName);
                        bool bCurrentStep = true;
                        if (Steps.xiStatus == 0)
                        {
                            string sBase = "";
                ...
                if (ostructureInstance.XIIValues.ContainsKey("Campaign Number for L2M use only"))
                {
                    ...
                    if (L2Museonly >= 17 && L2Museonly <= 1223)
                    {
                        l2muse = true;
                    }
                    else
            ...
            if (ages && UKlicences && startdt && Valueofyourvehicles && registeredkeepers && quotes && l2muse)
            {
