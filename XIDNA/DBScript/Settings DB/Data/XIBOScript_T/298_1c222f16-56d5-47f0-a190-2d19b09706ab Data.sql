 ...
 N'        public static CResult PolicyMainCal(List<CNV> lParam)
        {
            ...
            oResult.sMessage = "Static Carvan MTA script running";
        ...
        public class PolicyCalculation
        {
            public CResult Calculation(int iInsatnceID, string dCoverStart, string rGrossPremium, string QuoteID, string sSessionID, string sGUID, string sVersion)
            {
                ...
                try
                {
                    ...
                    var oQSI = oCache.Get_QsStructureObj(sSessionID, sGUID, "QSInstance_" + iInsatnceID + "StaticCaravan Quotes Structure");
                    var ostructureInstance = oQSI.oStructureInstance.FirstOrDefault().Value.FirstOrDefault();
                    //var oLIst = oQSI.oSubStructureI("Driver_T");
                    string dtInsuranceCoverStartDate = ostructureInstance.XIIValue("dtEffectiveFrom").sValue;
                    ...
                    var BalAmount = (Convert.ToDouble(rGrossPremium) / 365) * leftoverdays;
                    ...
                    if (QuoteStatus == "Normal")
                    {
                        ...
                        if (oProductI != null && oProductI.Attributes.ContainsKey("rMTACharge"))
                        {
                            if (double.TryParse(oProductI.Attributes["rMTACharge"].sValue, out rInsurerCharge))
                            {
                            }
                        }
                        rInsurerCharge += ((12 * 0.01) * rInsurerCharge);
                        if (total > 0)
                        {
                            total = total / 365 * leftoverdays - BalAmount;
                            oResult.sMessage = "MTA Premium:" + total;
                            oIB.SaveErrortoDB(oResult);
                            rExcLegalNIPT = total-10;
                            ...
                            oBII.Attributes["rSubTotal"] = new XIIAttribute { sName = "rSubTotal", sValue = String.Format("{0:0.00}", (total + rInsurerCharge)), bDirty = true };
                            ...
                            // {
                            //    rAdmin = Convert.ToDouble(oAdditionalCharges.oCollectionResult.Where(m => m.sName == "oResult").Select(m => m.sValue).FirstOrDefault());
                            oBII.Attributes["zDefaultAdmin"] = new XIIAttribute { sName = "zDefaultAdmin", sValue = String.Format("{0:0.00}", rAdmin), bDirty = true };
                            total += rAdmin + rInsurerCharge;
                            //}
                            oBII.Attributes["rInsurerCharge"] = new XIIAttribute { sName = "rInsurerCharge", sValue = String.Format("{0:0.00}", rInsurerCharge), bDirty = true };
                            ...
                            {
                                total += rPaymentCharge;
                            }
                        }

                        XIAPI oXIAPI = new XIAPI();
                        oBII.Attributes["rFinalQuote"] = new XIIAttribute { sName = "rFinalQuote", sValue = String.Format("{0:0.00}", total), bDirty = true };
                        ...
                        oBII.Attributes["rPaymentCharge"] = new XIIAttribute { sName = "rPaymentCharge", sValue = String.Format("{0:0.00}", rPaymentCharge), bDirty = true };
