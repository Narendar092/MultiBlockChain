INSERT INTO [XIBOScript_T] ([ID],
 ...
 [FKiTagSpaceIDXIGUID],
 [FKiVersionIDXIGUID]) Values(N'246',
 N'296',
 N'0',
 NULL,
 N'Zenith Marque Calculation_30-09-2019',
 N'PolicyCalculation',
        ...
        public class PolicyBaseCalc
        {
            ...
            public CResult GetGaragingLoad(string Area, string sTypeOfCover, string sGaraging)
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(oAreaLoad) && oAreaLoad.ToLower() == "decline")
                    {
                        oNV.sValue = xiEnumSystem.xiEnumPolicyLookupResponses.Decline.ToString();
                    }
                    else
            ...
            public CResult GetOccupationLoad(string sOccupation, string sTypeOfMotorhome, string sVehicleUse)
            {
                XIInstanceBase oIB = new XIInstanceBase();
                CResult oresult = new CResult();
                XIAPI oXIAPI = new XIAPI();
            ...
            public CResult GetConvictionsDemeritPoints(int NoOfConvictions, List<XIIBO> ConvictionI, string PolicyStartDate, int iProductID)
            {
                ...
                try
                {
                    ...
                    foreach (var Conviction in ConvictionI)
                    {
                        ...
                        //
                        if (Array.IndexOf(DeclineCodesWithinLastThreeeYrs, Prefix) > -1 && dDAte >= sPolicyStartDAte.AddYears(-3) && dDAte <= sPolicyStartDAte)
                        {
                            sDemeritpoints = "30";
                        }
            ...
            public CResult GetOwnerShipLoad(int iLengthOfOwnerShip, int iGroup, string sTypeOfMotorhome)
            {
                ...
                try
                {
                    ...
                    else if (iGroup >= 15 && iGroup <= 20 && sTypeOfMotorhome == "COMP")
                    {
                        if (iLengthOfOwnerShip < 12)
                        {
                            oNV1.sValue = "+13.00";
            ...
            public CResult GetProtectedNoClaimBonus(string IsProtectedNoClaimBonus, int iNoClaimYears)
            {
                XIInstanceBase oIB = new XIInstanceBase();
                CResult oresult = new CResult();
                try
            ...
            public CResult Calculation(int iInsatnceID, int iUserID, int iCustomerID, string sDataBase, string sProductName, string sVersion, string sProductCode, string sSessionID, string sUID, int iQuoteID)
            {
                ...
                try
                {
                    ...
                    foreach (var item in oDriversI)
                    {
                        ...
                        result = PolicyCal.GetOccupationLoad(sOccupation,TypeOfCover,sUseOfVechile);
                        ...
                        if (!string.IsNullOrEmpty(sSecondaryOccupation))
                        {
                            ...
                            result = PolicyCal.GetOccupationLoad(sSecondaryOccupation, TypeOfCover, sUseOfVechile);
                            if (result.xiStatus == 00 && result.oCollectionResult.Count > 0)
                            {
                                OccupationResult.Add(result);
                            }
                    ...
                    var oDistrict = PolicyCal.GetDisrictLoad(sArea, TypeOfCover);
                    if (oDistrict.bOK && oDistrict.oCollectionResult.Count > 0)
                    {
                        oXiResults.Add(oDistrict);
                    }
                    //Area
                    var AreaLoad = PolicyCal.GetAreaLoad(sArea, TypeOfCover);
                    if (AreaLoad.bOK && AreaLoad.oCollectionResult.Count > 0)
                    {
                        oXiResults.Add(AreaLoad);
                    }
                    //Garaging
                    var oGaragingLoad = PolicyCal.GetGaragingLoad(sArea, TypeOfCover, sGaraging);
                    ...
                    var oProtectedNoClaimBonus = PolicyCal.GetProtectedNoClaimBonus(sLiketoProtectYourNoClaimsDiscountUpdated, iNoClaimYears);
                    ...
                    double total = 0;
                    double BaseLoad = AdditionalExcess;
                    ...
                    else if (oXiResults.Any(m => m.oCollectionResult.Where(x => x.sName == "sMessage").Any(x => x.sValue == xiEnumSystem.xiEnumPolicyLookupResponses.Decline.ToString())) || oMinimumPremium.oCollectionResult.Where(m => m.sName == "sMessage").Select(m => m.sValue).FirstOrDefault() == xiEnumSystem.xiEnumPolicyLookupResponses.Decline.ToString() || oMedicalConditionLoad.oCollectionResult.Where(m => m.sName == "sMessage").Select(m => m.sValue).FirstOrDefault() == xiEnumSystem.xiEnumPolicyLookupResponses.Decline.ToString() || oEmploymentType.oCollectionResult.Where(m => m.sName == "sMessage").Select(m => m.sValue).FirstOrDefault() == xiEnumSystem.xiEnumPolicyLookupResponses.Decline.ToString() || oGeneralDeclines.Count > 0)
                    {
