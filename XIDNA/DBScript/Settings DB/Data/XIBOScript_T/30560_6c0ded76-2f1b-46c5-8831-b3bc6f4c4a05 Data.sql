Delete from XIBOScript_T where xiguid='6c0ded76-2f1b-46c5-8831-b3bc6f4c4a05';
INSERT INTO [XIBOScript_T] ([ID],
 ...
 [FKiTagSpaceIDXIGUID],
 [FKiVersionIDXIGUID]) Values(N'30560',
 N'296',
 N'0',
 NULL,
 N'AClass Business',
...
public class PolicyCalculation
{
    public CResult Calculation(string iInsatnceID, int iUserID, int iCustomerID, string sVersion, string sProductCode, int iQuoteID, int iOrgID, int ProductID)
    {
        ...
        Info.Add("AIB Insurance script running");
        XIInstanceBase oIB = new XIInstanceBase();
        CResult oresult = new CResult();
        ...
        try
        {
            ...
            List<bool> ArrayList = new List<bool>();
            //FIrst page 
            ...
            if (ostructureInstance.XIIValues.Count() > 0)
            {
                ...
                {
                    int UKlicence = Convert.ToInt32(ostructureInstance.XIIValues["sHowlonghaveyouheldaUKlicence"].sResolvedValue.Replace(" Years", ""));
                    if (UKlicence >= 4)
                    {
                        bProductValidation = true;
                    }
                ...
                if (ostructureInstance.XIIValues.ContainsKey("dWhenwouldyoulikethepolicytostart"))
                {
                    var policytostartdt = Convert.ToDateTime(ostructureInstance.XIIValues["dWhenwouldyoulikethepolicytostart"].sValue);
                    var Pdt = Convert.ToDateTime(policytostartdt.ToString("yyyy-MM-dd"));
                    DateTime d1 = DateTime.Now;
                    var Curdt = Convert.ToDateTime(d1.ToString("yyyy-MM-dd"));
                    int days = Convert.ToInt32((Pdt - Curdt).TotalDays);
                    if (days < 25)
                    {
                        bProductValidation = true;
                    }
                    ...
                    {
                        ArrayList.Add(false);
                        var Steps = BaseCal.PageValidation(ostructureInstance.XIIValues["Noofdays"].sDisplayName);
                        bool bCurrentStep = true;
                        if (Steps.xiStatus == 0)
                        {
                            string sBase = "";
                ...
                if (ostructureInstance.XIIValues.ContainsKey("sPleasecanyouconfirmthevalueofyourvehicle"))
                {
                    ...
                    if (Valueofyourvehicle == "Please select") { }
                    else
                    {
                ...
                if (ostructureInstance.XIIValues.ContainsKey("MTPostcode"))
                {
                    ...
                    {
                        ArrayList.Add(false);
                        var Steps = BaseCal.PageValidation("Post code exclude");
                        bool bCurrentStep = true;
                        if (Steps.xiStatus == 0)
                        {
                            string sBase = "";
            ...
            if (ArrayList != null && ArrayList.All(c => c == false))
            {
                bProductValidation = false;
            }
 ...
 NULL,
 NULL,
 N'6c0ded76-2f1b-46c5-8831-b3bc6f4c4a05',
 N'0',
 N'',
 N'',
 N'2024-12-09 10:18:33.000',
