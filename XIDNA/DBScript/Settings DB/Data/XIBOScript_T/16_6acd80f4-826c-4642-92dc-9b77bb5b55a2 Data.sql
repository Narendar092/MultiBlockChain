INSERT INTO [XIBOScript_T] ([ID],
 ...
 [FKiTagSpaceIDXIGUID],
 [FKiVersionIDXIGUID]) Values(N'16',
 N'296',
 N'0',
 NULL,
 N'PolicyCalculation',
 ...
 N' public static CResult PolicyMainCal(List<CNV> lParam)
        ...
        public abstract class PolicyBaseCalc
        {
            ...
            //public CResult GetDrivingRestrictionsLoadFactor(string DrivingRestriction,int DriverSno)
            public CResult GetDrivingRestrictionsLoadFactor(string DrivingRestriction)
            {
                CResult oresult = new CResult();
                try
                {
                    ...
                    if (DrivingRestriction.ToLower() == "policyholder")
                    {
                        oNV1.sValue = "-15";
                    }
                    else if (DrivingRestriction.ToLower() == "spouse")
                    {
                        oNV1.sValue = "-25";
                    }
                    oresult.oCollectionResult.Add(oNV);
            ...
            public CResult GetClaimLoadFactor(int NoOfClaims, string ClaimCost,List<int>YearOfClaims)
            {
                CResult oresult = new CResult();
                try
                {
                    ...
                    if ((NoOfClaims == 1 && Convert.ToInt32(ClaimCost) <= 10000) || !YearOfClaims.Any(m=>m >3))
                    {
                        oNV.sValue = xiEnumSystem.xiEnumPolicyLookupResponses.Normal.ToString();
                    }
                    oresult.oCollectionResult.Add(oNV);
        ...
        public class PolicyCalculation
        {
            public CResult Calculation(int iInsatnceID, int iUserID, int iCustomerID, string sDataBase, string sProductName,string sProductCode, string sVersion, string sSessionID, string sUID)
            {
                ...
                try
                {
                    ...
                    List<CResult> DrivingDiscountResult = new List<CResult>();
                   ...
                   // oBII.BOD = (XIDBO)oXIDXI.Get_BODefinition("Aggregations").oResult;
                    ...
                    foreach (var item in oDriversI)
                    {
                        ...
                        var sRelation = item.AttributeI("enumRelationship").sResolvedValue;
                        oResult.sMessage = "sRelation_" + sRelation;
                        oIB.SaveErrortoDB(oResult);
                        if (!string.IsNullOrEmpty(sRelation))
                        {
                            result = PolicyCal.GetDrivingRestrictionsLoadFactor(sRelation);
                            //result=PolicyCal.GetDrivingRestrictionsLoadFactor(sRelation,j);
                            if (result.xiStatus == 0 && result.oCollectionResult.Count > 0)
                            {
                                DrivingDiscountResult.Add(result);
                            }
                        }
                        ...
                        foreach (var oClaim in sSubList)
                        {
                            ...
                            {
                                result = PolicyCal.GetAgeFromDOB(oClaim.AttributeI("dDate").sValue, DateTime.Now.ToString());
                                if (result.xiStatus == 00 && result.oResult != null)
                                {
                        ...
                        if (NoOfClaims > 0)
                        {
                            result = PolicyCal.GetClaimLoadFactor(NoOfClaims, rTotalCost.ToString(), claimYearList);
                            if (result.xiStatus == 0 && result.oCollectionResult.Count > 0)
                            {
                    ...
                    //}
                    var oDrivingDiscountLoadFactor = (PolicyCal.GetMaxLoadFactor(DrivingDiscountResult));
                    if (oDrivingDiscountLoadFactor.xiStatus == 0 && oDrivingDiscountLoadFactor.oCollectionResult != null)
                    {
                        oXiResults.Add(oDrivingDiscountLoadFactor);
                    }
                    ...
                    if (oXiResults.All(m => m.oCollectionResult.Where(x => x.sName == "sMessage").All(x => x.sValue == xiEnumSystem.xiEnumPolicyLookupResponses.Normal.ToString())) && oExcessVal.oCollectionResult.Where(m => m.sName == "sMessage").Select(m => m.sValue).FirstOrDefault() == xiEnumSystem.xiEnumPolicyLookupResponses.Normal.ToString() && oBaseLoadFactor.oCollectionResult.Where(m => m.sName == "sMessage").Select(m => m.sValue).FirstOrDefault() == xiEnumSystem.xiEnumPolicyLookupResponses.Normal.ToString() && IPTLoadFactor.oCollectionResult.Where(m => m.sName == "sMessage").Select(m => m.sValue).FirstOrDefault() == xiEnumSystem.xiEnumPolicyLookupResponses.Normal.ToString() && LeftHandLoadFactor.oCollectionResult.Where(m => m.sName == "sMessage").Select(m => m.sValue).FirstOrDefault() == xiEnumSystem.xiEnumPolicyLookupResponses.Normal.ToString())
                    {
                        ...
                        var miniumDepositPercent = oProductI.Attributes["zDefaultDeposit"].sValue;
                        var miniumDepositAmount = oProductI.Attributes["rDefaultAmount"].sValue;
                        var DepositType= oProductI.Attributes["iDefaultAmountType"].sResolvedValue;
                        double MinimumDepositPercentage = 0;
                        double MinimumDepositAmount = 0;
                        if (DepositType == "Percentage")
                        {
                            if (double.TryParse(miniumDepositPercent, out MinimumDepositPercentage))
                            {
                                MinimumDepositAmount = (total * 0.01) * MinimumDepositPercentage;
                            }
                        }
                        else
                        {
                            if (double.TryParse(miniumDepositAmount, out MinimumDepositAmount))
                            {
                            }
                        }
                        ...
                        oBII.Attributes["zDefaultDeposit"] = new XIIAttribute { sName = "zDefaultDeposit", sValue = MinimumDepositAmount.ToString(), bDirty = true };
                        ...
                        { }
                        var MonthlyAmount=oXIAPI.GetMonthlyPremiumAmount(total, iProductID);
                        oResult.sMessage = "Monthly Amount:"+ MonthlyAmount;
                        oIB.SaveErrortoDB(oResult);
                    ...
                    oBII.Attributes["sRefID"] = new XIIAttribute { sName = "sRefID", sValue = Guid.NewGuid().ToString("N").Substring(0, 6), bDirty = true };
                    ...
                    oIB.SaveErrortoDB(oResult);
                    XIIBO oBO = new XIIBO();
                    oBO.BOD = (XIDBO)oCache.GetObjectFromCache(XIConstant.CacheBO, "RiskFactor_T");
