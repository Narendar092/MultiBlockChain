Delete XIBOScript_T where XIGUID='e692feed-75ed-4b0a-b191-fd0be12678ad'
INSERT INTO [XIBOScript_T] ([ID],
 ...
 [FKiTagSpaceIDXIGUID],
 [FKiVersionIDXIGUID]) Values(N'30556',
 N'296',
 N'0',
 NULL,
 N'AIB INSURENCE',
    ...
    public CResult Calculation(string iInsatnceID, int iUserID, int iCustomerID, string sVersion, string sProductCode, int iQuoteID, int iOrgID, int ProductID)
    {
        ...
        try
        {
            ...
            if (ostructureInstance.XIIValues.Count() > 0)
            {
                ...
                if (ostructureInstance.XIIValues.ContainsKey("sCanyouconfirmyouremploymentstatus"))
                {
                    int employmentstatus = Convert.ToInt32(ostructureInstance.XIIValues["sCanyouconfirmyouremploymentstatus"].sValue);
                    if (employmentstatus == 20 || employmentstatus == 40)
                    {
                        employmentstatu = true;
                    }
                    else
                    {
                        bProductValidation = false;
                        var Steps = BaseCal.PageValidation(ostructureInstance.XIIValues["sCanyouconfirmyouremploymentstatus"].sDisplayName);
                        bool bCurrentStep = true;
                        if (Steps.xiStatus == 0)
                        {
                            string sBase = "";
                ...
                {
                    var days = Convert.ToInt32(ostructureInstance.XIIValues["Noofdays"].sValue);
                    if (days < 25)
                    {
                        startdt = true;
                    }
                ...
                if (ostructureInstance.XIIValues.ContainsKey("Campaign Number for L2M use only"))
                {
                    ...
                    if (L2Museonly >= 10 && L2Museonly <= 1115)
                    {
                        l2muse = true;
                    }
                    else
            ...
            if (ages && startdt && Valueofyourvehicles && employmentstatu && postcode && l2muse)
            {
