    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            try
            {
                ...
                var newxx = SessionManager.sUserName;
                ...
                var EncryptedPwd2 = oEncrypt.EncryptData("Data Source==WIN-IN1I7P2MED3\\SQLEXPRESS01;initial catalog=MBCO_XIEnvironment;User Id=XIDNAADMIN; Password=XIDNAADMIN; MultipleActiveResultSets=True", true, "6105A023-04DF-40A8-9669-69E75D4A6B50".ToLower());
                var EncryptedPwd3 = oEncrypt.EncryptData("Data Source==WIN-IN1I7P2MED3\\SQLEXPRESS01;initial catalog=MBCO_Settings;User Id=XIDNAADMIN; Password=XIDNAADMIN; MultipleActiveResultSets=True", true, "D2EF9017-0D1C-44CA-A2C0-AD0527BAAC5B".ToLower());
                var EncryptedPwd4 = oEncrypt.EncryptData("Data Source==WIN-IN1I7P2MED3\\SQLEXPRESS01;initial catalog=MBCO_Core;User Id=XIDNAADMIN; Password=XIDNAADMIN; MultipleActiveResultSets=True", true, "A428DFD2-FCAB-409E-802E-D6746A75CC60".ToLower());
                var EncryptedPwd5 = oEncrypt.EncryptData("Data Source==WIN-IN1I7P2MED3\\SQLEXPRESS01;initial catalog=MBCO_Org;User Id=XIDNAADMIN; Password=XIDNAADMIN; MultipleActiveResultSets=True", true, "32702371-2853-4F84-99CE-0EE4726E39B8".ToLower());
                ...
                {
                    oUser.FKiOrgID = SessionManager.OrganizationID;
                }
                ...
                if (oUser.XIDeleted == 0 && oUser.StatusTypeID == 10)
                {
                    ...
                    XIDXI oXIID = new XIDXI();
                    var oUserPreference = oXIID.Get_UserPreference(oUser.UserID.ToString(), Database.ToString());
                    if (oUserPreference.oResult != null)
                    {
                        SessionManager.UserPreference = (XIInfraUserPreference)oUserPreference.oResult;
                    }
                    else { }
                    ...
                    if (oUser != null && oUser.Role != null && oUser.Role.RoleID != 55)
                    {
                        ...
                        var sReferenceID = (string)oCache.GetObjectFromCache(XIConstant.CacheConfig, oUser.FKiApplicationID + "_" + oUser.FKiOrgID + "_" + oUser.Role.RoleID + "_" + XIConstant.IDRef_internal);
                        SessionManager.sReference = sReferenceID;
                        bool bOTP = false;
                        XIDApplicationOTPSetting oOTPSetting = (XIDApplicationOTPSetting)oCacheO.GetObjectFromCache(XIConstant.CacheApplicationOTP, null, oUser.FKiApplicationIDXIGUID.ToString());
                        if (oOTPSetting.iStatus == 10)
                        {
                            bOTP = true;
                        }
                        ...
                        {
                            var authCookiee = HttpContext.Request.Cookies[FormsAuthentication.FormsCookieName];
                            if (authCookiee != null)
                            {
                                FormsAuthenticationTicket authTickett = FormsAuthentication.Decrypt(authCookiee.Value);
                                if (authTickett != null && !authTickett.Expired)
                                {
                                    var roles = authTickett.UserData.Split(',');
                                    HttpContext.User = new System.Security.Principal.GenericPrincipal(new FormsIdentity(authTickett), roles);
                                }
                            }
                            if (authCookiee == null && bOTP)
                            {
                                GenerateOTPToUser(oUser, iMainApplicationID);
                                oUser.sOTP = sOTP;
                                ...
                                oConfig.Save_User(oUser, DataSourceXIGUID.ToString());
                            ...
                            //else
                            if (oUser.Role.iThemeID > 0 || (oUser.Role.iThemeIDXIGUID != null && oUser.Role.iThemeIDXIGUID != Guid.Empty))
                            {
                                if (oUser.Role.iThemeID > 0 || (oUser.Role.iThemeIDXIGUID != null && oUser.Role.iThemeIDXIGUID != Guid.Empty))
                                {
                                ...
                                if (oUser.Role.bSignalR)
                                {
                                    ...
                                    nParams.Add(new CNV() { sName = "{XIP|iOrgID}", sValue = oUser.FKiOrgID.ToString() });
                                ...
                                Singleton.Instance.sOrgID = oUser.FKiOrgID.ToString();
                                ...
                                SessionManager.OrganizationID = oUser.FKiOrgID;
                                SessionManager.iUserOrg = oUser.FKiOrgID;
                                ...
                                SessionManager.ApplicationIDXIGUID = oUser.FKiApplicationIDXIGUID;
                                SessionManager.iUserLevel = oUser.iLevel;
                                SessionManager.sTeam = oUser.sTeamHierarchy;
                                SessionManager.bDebug = oUser.bDebug;
                                SessionManager.iLogLevel = oUser.iLogLevel;
                                ...
                                //Session.Abandon();
                                if (oUser.TwoFactorEnabled)
                                {
                                    EnableTOTPAuthenticatorViewModel obj = new EnableTOTPAuthenticatorViewModel();
                                    obj.ManualEnterKey = oUser.sTotpSecretKey.ToString();
                                    return View("LoginPageWith2FA", obj);
                                }
                                else
                                {
                                    oCResult.sMessage = "INFO: Login - Successful login for user:" + oUser.sUserName + " on " + DateTime.Now;
                                    oCResult.sCategory = "Successful  login";
                                    oXID.SaveErrortoDB(oCResult);
                                    return RedirectToAction("LandingPages", "Home");
                                }
                                //return RedirectToAction("LandingPages", "Home");
                            }
                            //if (oUser.sTotpSecretKey != null)
                            if (oUser.TwoFactorEnabled)
                            {
                                EnableTOTPAuthenticatorViewModel objs = new EnableTOTPAuthenticatorViewModel();
                                objs.ManualEnterKey = oUser.sTotpSecretKey.ToString();
                                return View("LoginPageWith2FA", objs);
                            }
                            else
                            {
                                oCResult.sMessage = "INFO: Login - Successful login for user:" + oUser.sUserName + " on " + DateTime.Now;
                        ...
                        {
                            oCResult.sMessage = "INFO: Login - There is a failed login attempt for username:" + oUser.sUserName + " on " + DateTime.Now;
                            oCResult.sCategory = "Failed login";
                            oXID.SaveErrortoDB(oCResult);
            ...
            {
                oCResult.sMessage = "INFO: Login - There is a failed login attempt for username:" + model.UserName + " on " + DateTime.Now;
                oCResult.sCategory = "Failed login";
                oXID.SaveErrortoDB(oCResult);
        ...
        public ActionResult GenerateOTPToUser(XIInfraUsers oUserDetails, int AppliationID, XIIBO SendingType = null, string sType = null, string sResendOTP = null)
        {
            ...
            int iOtpLength = 0;
            XIInfraCache oCache = new XIInfraCache();
            XIDApplicationOTPSetting oOTP = (XIDApplicationOTPSetting)oCache.GetObjectFromCache(XIConstant.CacheApplicationOTP, null, AppliationID.ToString());
            ...
            {
                sEmail = oOTP.bMail.ToString(); //SendingType.Attributes.Where(u => u.Key.ToLower() == "bmail").Select(y => y.Value.sValue).FirstOrDefault();
                sSMS = oOTP.bSMS.ToString(); //SendingType.Attributes.Where(u => u.Key.ToLower() == "bsms").Select(y => y.Value.sValue).FirstOrDefault();
                var OTPType = oOTP.iOTPType; //SendingType.Attributes.Where(u => u.Key.ToLower() == "iotptype").Select(y => y.Value.sValue).FirstOrDefault();
                var OTPCase = oOTP.iOTPCase; //SendingType.Attributes.Where(u => u.Key.ToLower() == "iotpcase").Select(y => y.Value.sValue).FirstOrDefault();
                var iOTPLength = oOTP.iOTPLength; //SendingType.Attributes.Where(u => u.Key.ToLower() == "iotplength").Select(y => y.Value.sValue).FirstOrDefault();
        ...
        [HttpPost]
        [AllowAnonymous]
        public ActionResult LoginPageWith2FA(EnableTOTPAuthenticatorViewModel model)
        {
            if (ModelState.IsValid)
            {
                var status = XIDNATOTPGenerator.VerifyOTP(model.ManualEnterKey, model.OTP);
                if (status)
                {
                    return RedirectToAction("LandingPages", "Home");
                }
            ...
            ModelState.AddModelError("", "Input filed is required");
            return View(model);
        }
        [HttpPost]
        [AllowAnonymous]
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            ...
            try
            {
                ...
                if (oContentDef != null && oContentDef.Count() > 0)
                {
                    ...
                    oBOI.SetAttribute("sname", oUser.sFirstName);
                    ...
                    XIIBO oCommuncationBOI = new XIIBO();
                    XIDBO oCommuncationBOD = (XIDBO)oCache.GetObjectFromCache(XIConstant.CacheBO, "XICommunicationI_T", null);
                    oCommuncationBOI.BOD = oCommuncationBOD;
                    oCommuncationBOI.SetAttribute("sContent", (string)oRes.oResult);
                    oCommuncationBOI.SetAttribute("FKiCommunicationTypeIDXIGUID", "02E1DD72-E393-4E56-9D7E-758415C3F087");
                    oCommuncationBOI.SetAttribute("iComType", "10");
                    oCommuncationBOI.SetAttribute("iDirection", "20");
                    oCommuncationBOI.SetAttribute("sHeader", "Forgot Password");
                    oCommuncationBOI.SetAttribute("sTo", oUser.sEmail);
                    oCommuncationBOI.Save(oCommuncationBOI);
                    var iCommuncationID = oCommuncationBOI.AttributeI("id").sValue;
                    List<CNV> oComParams = new List<CNV>();
                    XIInfraCommunicationComponent oComm = new XIInfraCommunicationComponent();
                    CResult oCR = new CResult();
                    oComParams.Add(new CNV { sName = "iInstanceID", sValue = iCommuncationID });
                    ...
                    //oEmail.Sendmail(oUser.FKiOrgID, sContent, null, 0, XIConstant.Email_ForgotPassword, 0, null, 0, false, false, null, null, oEmail.iServerID);//send mail with attachment
        ...
        public async Task<ActionResult> ClientForgotPassword(ForgotPasswordViewModel model)
        {
            ...
            try
            {
                ...
                if (oContentDef != null && oContentDef.Count() > 0)
                {
                    ...
                    oEmail.EmailID = oUser.sEmail;
                    oEmail.sSubject = oDocumentContent.Name;
                    oEmail.Sendmail(oUser.FKiOrgID, sContent, null, 0, XIConstant.Email_ForgotPassword);//send mail with attachment
                    oCResult.sMessage = "Mail send successfully";
                    oParams = new List<CNV>();
        ...
        public ActionResult CheckSecureKey(string sSecureKey)
        {
            ...
            else if (Guid.TryParse(sSecureKey, out XILinkGUID))
            {
                return Json(XILinkGUID, JsonRequestBehavior.AllowGet);
            }
            else
            {
        ...
        public async Task<ActionResult> AdminResetPassword(ResetPasswordViewModel model)
        {
            ...
            {
                ModelState.AddModelError("", "Your password must Be a minimum of 8 characters long and include at least one uppercase, one lowercase ,one number and one special character.");
                return View("AdminForgotPassword", model);
            }
            return RedirectToAction("LandingPages", "Home");
        }
        ...
        public ActionResult UserResetPassword(string sEmail = "")
        {
            ...
            try
            {
                ...
                if (oContentDef != null && oContentDef.Count() > 0)
                {
                    ...
                    oEmail.EmailID = oUser.sEmail;
                    oEmail.sSubject = oDocumentContent.Name;
                    oEmail.Sendmail(oUser.FKiOrgID, sContent, null, 0, XIConstant.Email_ResetPassword);//send mail with attachment
                    oCResult.sMessage = "Mail send successfully";
                    oXID.SaveErrortoDB(oCResult);
                    oCResult.oTraceStack.Add(new CNV { sName = "Mail send successfully", sValue = "Mail send successfully" });
                }
        ...
        {
            var Database = SessionManager.CoreDatabase;
            var sUsername = SessionManager.sUserName;
            XIInfraUsers oUser = new XIInfraUsers();
            IndexViewModel prof = new IndexViewModel();
            oUser.sUserName = sUsername;
            oUser = (XIInfraUsers)oUser.Get_UserDetails(Database.ToString()).oResult;
            prof.HasPassword = true;
            prof.PhoneNumber = "9848022338";
            if (oUser.TwoFactorEnabled == true)
            {
                prof.TwoFactor = true;
            }
            else
            {
                prof.TwoFactor = false;
            }
            return PartialView("_ViewProfile", prof);
        }
